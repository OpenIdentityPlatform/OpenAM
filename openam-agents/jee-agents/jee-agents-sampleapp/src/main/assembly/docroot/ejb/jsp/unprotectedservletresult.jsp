...
<!--
   ...
   $Id: unprotectedservletresult.jsp,v 1.11 2008/08/19 19:13:42 veiming Exp $
...
<%
    ...
    String message = null;
    if (success) {
        String ejbAccessResult = (String) request.getAttribute("EJB-ACCESS");
        if (ejbAccessResult != null && ejbAccessResult.equals("OK")) {
            access = true;
        }
        String ejbMessage = (String) request.getAttribute("EJB-MESSAGE");
        if (ejbMessage != null) {
            message = ejbMessage;
        }
    }
...
<body>
<table style="width: 800px; text-align: left;" border="0"
 cellpadding="2" cellspacing="2">
  <tbody>
    <tr>
        <td style="background-color: white;"><a href="http://forgerock.com/"><img
                  alt="ForgeRock" src="/agentsample/images/logo.png"
                  style="width: 156px; height: 104px;"></a><br>
        </td>
        <td style="text-align: center; vertical-align: middle; font-family: andale sans;">
    ...
    <tr>
      ...
      <table style="text-align: left; width: 572px; height: 394px;" border="0" cellpadding="2" cellspacing="2">
		...
		<% if (success) { 
		      if (access) { %>
		         <br><b>Successful Invocation</b><br>
		      <% } else { %>
			 <br><b>Failed Invocation</b><br>
		     <% } %>
		<% } else { %>
		...
		<% } %>
          ...
          <tr>
            ...
            <td style="vertical-align: top;"> 
		<% if (success) { %>
			<br>This page was dispatched by an Unprotected Servlet which invokes an EJB that is protected by a
			declarative security constraint requiring the user to be in the role <code>employee</code>.<br><br>
			<% if (access) { %>
				 This invocation was successful. The EJB sent the message:<br><br>
				<code><%=message%></code><br>
			<% } else { %>
				This invocation has failed.
				<% if (message != null && message.trim().length() > 0) { %>
					 The message received was:<br><br> <code><%=message%></code><br>
				<% } else { %>
					No message was received.<br>
				<% } %>
			<% } %>
		<% } else { %>
		       <br> The dispatched page was not routed via the Unprotected Servlet. This
			condition can happen if the page was accessed directly instead of being
			routed via the Unprotected Servlet.<br>
		<% } %>
	        <br> You can return to 
		<a href="/agentsample/public/declarativesecurity.html">J2EE Declarative Security</a> page to try again.<br>
            </td>
            <td style="vertical-align: top; width: 75px;">&nbsp;<br>
            </td>
          </tr>
