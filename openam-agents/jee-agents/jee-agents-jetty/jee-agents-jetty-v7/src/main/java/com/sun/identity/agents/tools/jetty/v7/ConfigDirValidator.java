...
public class ConfigDirValidator extends ValidatorBase implements IConstants, IConfigKeys {
     ...
     *
     */
    public ValidationResult isConfigDirValid(String configDir, Map props, IStateAccess state) {
        ...
        if ((configDir != null) && (configDir.trim().length() >= 0)) {
            ...
            String serverXmlFile = configDir + STR_FORWARD_SLASH + STR_SERVER_XML;
            Debug.log("ConfigDirValidator.isConfigDirValid(): Server XML file path is: " + serverXmlFile);
        ...
        Debug.log("ConfigDirValidator.isConfigDirValid(): Is Jetty Server Config dir "
    ...
    private boolean isValid(String fileName) {
        ...
        if (file.exists() && file.canRead() && file.canWrite()) {
            ...
            if (!file.exists()) {
                Debug.log("ConfigDirValidator.isValid(): " + fileName + " does not exist");
            } else {
                Debug.log("ConfigDirValidator.isValid(): " + fileName + " does not have read and write permissions");
            }
        }
        return valid;
    }
    ...
    public void initializeValidatorMap() throws InstallException {
        ...
        try {
            getValidatorMap().put("VALID_JETTY_CONFIG_DIR", this.getClass().getMethod("isConfigDirValid", paramObjs));
        } catch (NoSuchMethodException nsme) {
            Debug.log("ConfigDirValidator: NoSuchMethodException thrown while loading method :", nsme);
            throw new InstallException(LocalizedMessage.get(LOC_VA_ERR_VAL_METHOD_NOT_FOUND), nsme);
        } catch (SecurityException se) {
            Debug.log("ConfigDirValidator: SecurityException thrown while loading method :", se);
            throw new InstallException(LocalizedMessage.get(LOC_VA_ERR_VAL_METHOD_NOT_FOUND), se);
        } catch (Exception ex) {
            Debug.log("ConfigDirValidator: Exception thrown while loading method :", ex);
