...
/**
 * <code>PPResetQuestionTiledView</code> is a tiled view for password reset question view bean.
 */
public class PPResetQuestionTiledView extends RequestHandlingTiledViewBase implements TiledView, RequestHandler {
	 ...
	 */
	public PPResetQuestionTiledView(View parent, String name) {
		super(parent, name);
	...
	public boolean nextTile() throws ModelControlException {
		...
		PPResetQuestionModel model = getModel();
		if (movedToRow) {
	 ...
	 */
	private PPResetQuestionModel getModel() {
		PPResetQuestionViewBean parentVB = (PPResetQuestionViewBean) getParentViewBean();
		return (PPResetQuestionModel) parentVB.getModel();
	}
	...
	public void populateQuestionsList(String userDN, String orgDN) {
		PPResetQuestionModel model = getModel();
		int maxQuestions = new UMUserPasswordResetOptionsModelImpl(null, map)
		...
		if (secretMap != null && !secretMap.isEmpty()) {
			list = new ArrayList<String>(secretMap.keySet());
			Collections.shuffle(list);
			if (list.size() > maxQuestions)
				list = list.subList(0, maxQuestions);
		} else {
	...
	public Map<String, String> getAnswers() {
		...
		try {
			...
			model.debugError("PPResetQuestionTiledView.getAnswers", mce);
		}
		return map;
	}
