 ...
 *
 * $Id: PPResetModelImpl.java,v 1.3 2008/06/25 05:43:42 qcheng Exp $
 *
...
/**
 * <code>PPResetModelImpl</code> defines the basic and commonly used methods
 * used by view beans.
 */
@SuppressWarnings("unchecked")
public class PPResetModelImpl implements PPResetModel {

	/**
	 * Password reset Debug file name
	 */
	private static final String PASSPHRASE_DEBUG_FILENAME = "amPassphraseReset";
	 ...
	 * Debug object
	 */
	public static Debug debug = Debug.getInstance(PASSPHRASE_DEBUG_FILENAME);
	 ...
	 */
	protected String passphraseResetMsg = null;

	/**
	 * Logger object
	 */
	protected PPResetAdminLog logger = null;
	...
	public String getLocalizedString(String key) {
		...
		try {
			ResourceBundle rb = PPResetResBundleCacher.getBundle(DEFAULT_RB, localeContext.getLocale());
			i18nString = rb.getString(key);
		} catch (MissingResourceException e) {
			debug.warning("PPResetModelImpl.getLocalizedString", e);
		}
		return i18nString;
	}
	 ...
	 * @param e <code>printStackTrace</code> will be invoked to print the
	 *            stack trace.
	 */
	...
	public String getHTMLPageTitle() {
		return getLocalizedString("ppConsole.title");
	}
	 ...
	 */
	public String getPassphraseResetMessage() {
		return passphraseResetMsg;
	}

	/**
	 * Returns a localized error message from an exception. If the exception is
	 * of type <code>AMException</code> the error code and any possible
	...
	protected ServiceSchema getPWResetServiceSchema() throws SSOException, SMSException {
		ServiceSchemaManager mgr = new ServiceSchemaManager(PW_RESET_SERVICE, ssoToken);
	...
	protected String getL10NAttributeName(ServiceSchemaManager mgr, String key) {
		...
		try {
			...
			if (name != null) {
				ResourceBundle rb = PPResetResBundleCacher.getBundle(name, localeContext.getLocale());
				i18nName = Locale.getString(rb, key, debug);
			}
		} catch (MissingResourceException e) {
			debug.warning("PPResetModelImpl.getL10NAttributeName", e);
		}
		return i18nName;
	}
	...
	public String getL10NAttributeName(String serviceName, String key) {
		...
		try {
			...
			if (debug.warningEnabled()) {
				debug.warning("PPResetModelImpl.getL10NAttributeName: "
						+ "Could not localized str for " + key + " in service " + serviceName, mre);
			}
	...
	/**
	 * Initializes the data for this model by getting locale, SSO Token and
	 * <code>AMStoreConnection</code>
	 */
	private void initialize() {
		ssoToken = getSSOToken();
		logger = new PPResetAdminLog(ssoToken);
		resBundle = PPResetResBundleCacher.getBundle(rbName, localeContext.getLocale());
	}

	/**
	 * Sets the password reset enabled flag depending what is set in the
	 * password service for a given realm.
	 * 
	 * @param realm Realm
	 */
	public void readPWResetProfile(String realm) {
		...
		try {
			...
			debug.warning("PPResetModelImpl.readPWResetProfile", e);
		} catch (SMSException e) {
			debug.error("PPResetModelImpl.readPWResetProfile", e);
		}
		if ((value == null) || !value.equals(STRING_TRUE)) {
			pwResetEnable = false;
			informationMsg = getLocalizedString("ppResetDisabled.message");
		}
	}
	...
	/**
	 * Returns the first <code>String</code> element from the given set. If the
	 * set is empty, or null, an empty string will be returned.
	 * 
	...
	/**
	 * Populates the password reset lockout attributes for a given organization
	 * distinguished name and stored them in the model.
	 * 
	...
	public void populateLockoutValues(String orgDN) {
		try {
			...
			pwResetLockoutNotification = getAttributeValue(orgDN, PW_RESET_LOCKOUT_EMAIL_ADDRESS);
			...
			debug.error("PPResetModelImpl.populateLockoutValues", e);
		} catch (SSOException e) {
			debug.warning("PPResetModelImpl.populateLockoutValues", e);
		} catch (SMSException e) {
			debug.error("PPResetModelImpl.populateLockoutValues", e);
		}
	}
	...
	public boolean isUserLockout(String userDN, String orgDN) {
		...
		PPResetAccountLockout pwResetLockout = new PPResetAccountLockout(this);
