...
/**
 * <code>PPResetAccountLockout</code> defines a set for methods to lock and
 * unlock the user's account if their attempt to reset password were
 * unsuccessful after n of tries.
 */
public class PPResetAccountLockout {
	private static Map<String, AccountLockoutInfo> ppResetFailHash = Collections.synchronizedMap(new HashMap<String, AccountLockoutInfo>());
	private int userWarningCount = 0;
	private PPResetModelImpl model = null;
	 ...
	 */
	public PPResetAccountLockout(PPResetModelImpl model) {
		this.model = model;
	 ...
	 * <code>AccountLockInfo</code> object. It will remove the user from hash
	 * map if user is physically locked out.
	 * 
	...
	public void invalidAnswer(AMIdentity amid) {
		...
		AccountLockoutInfo pwLockoutInfo = (AccountLockoutInfo) ppResetFailHash.get(amid.getUniversalId());
		...
		if (isAccountLockout.isAccountLocked(amid)) {
			ppResetFailHash.remove(amid.getUniversalId());
			userWarningCount = -1;
		} else {
			ppResetFailHash.put(amid.getUniversalId(), pwLockoutInfo);
		}
	}
	...
	public int getWarnUserCount(String userDN) {
		AccountLockoutInfo acInfo = (AccountLockoutInfo) ppResetFailHash.get(userDN);
		return (acInfo != null) ? acInfo.getWarningCount() : userWarningCount;
	}
