...
public class RiskBasedSingleProfileMatcher {
	
	private RiskBasedDevicePrintComparator devicePrintComparator = new RiskBasedDevicePrintComparator();
	
	public ComparisonResult matchDevicePrintAgainstProfile(
			UserProfile userProfile, DevicePrint currentDevicePrint, RiskBasedRunnerConfig config) {
		DevicePrint storedDevicePrint = userProfile.getDevicePrint();
		
		return devicePrintComparator.compareDevicePrintsPart(currentDevicePrint, storedDevicePrint, config);
	}
