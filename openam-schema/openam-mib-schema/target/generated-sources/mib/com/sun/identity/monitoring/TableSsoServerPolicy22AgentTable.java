 ...
 * The group is defined with the following oid: 1.3.6.1.4.1.42.2.230.3.1.1.2.1.21.5.
 */
public class TableSsoServerPolicy22AgentTable extends SnmpTableSupport implements Serializable {

    /**
     * Constructor for the table. Initialize metadata for "TableSsoServerPolicy22AgentTable".
     ...
     */
    public TableSsoServerPolicy22AgentTable(SnmpMib myMib) {
        super(myMib);
    }
     ...
     */
    public TableSsoServerPolicy22AgentTable(SnmpMib myMib, MBeanServer server) {
        this(myMib);
        this.server = server;
    }
    ...
    public void createNewEntry(SnmpMibSubRequest req, SnmpOid rowOid,
        ...
        try  {
            ...
            // the returned object must implement the "SsoServerPolicy22AgentEntryMBean"
            // interface.
            //
            final Object entry =
                 createSsoServerPolicy22AgentEntryMBean(req, rowOid, depth, objname, meta,
                    oid0.toInteger(),
                    oid1.toInteger());
            if (server != null) {
    ...
    protected SnmpMibTable getRegisteredTableMeta(SnmpMib mib)  {
        return mib.getRegisteredTableMeta("SsoServerPolicy22AgentTable");
    }
    ...
    public synchronized void addEntry(SsoServerPolicy22AgentEntryMBean entry)
        throws SnmpStatusException {
    ...
    public synchronized void addEntry(SsoServerPolicy22AgentEntryMBean entry, ObjectName name)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
    ...
    public synchronized SsoServerPolicy22AgentEntryMBean[] getEntries() {
        Object[] array = getBasicEntries();
        SsoServerPolicy22AgentEntryMBean[] result = new SsoServerPolicy22AgentEntryMBean[array.length];
    ...
    public void removeEntry(SsoServerPolicy22AgentEntryMBean entry)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
    ...
    public ObjectName buildNameFromIndex(SnmpIndex index)
        ...
        try  {
            ...
            String _keyPolicy22AgentIndex = oid.toInteger().toString();
            return new ObjectName("TableSsoServerPolicy22AgentTable:name=com.sun.identity.monitoring.SsoServerPolicy22AgentEntry" + ",SsoServerRealmIndex=" + _keySsoServerRealmIndex + ",Policy22AgentIndex=" + _keyPolicy22AgentIndex);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.snmpRspWrongValue);
        } catch(Exception e) {
            throw new SnmpStatusException(e.getMessage());
     ...
     */
    public SnmpIndex buildSnmpIndex(SsoServerPolicy22AgentEntryMBean entry)
        throws SnmpStatusException {
        SnmpOid[] oids = new SnmpOid[2];
        SnmpValue val = null;
        val = new SnmpInt(entry.getSsoServerRealmIndex());
        oids[0] = val.toOid();
        val = new SnmpInt(entry.getPolicy22AgentIndex());
        oids[1] = val.toOid();
     ...
     * Build index for "SsoServerPolicy22AgentEntry".
     */
    public SnmpOid buildOidFromIndexVal(Integer aSsoServerRealmIndex, Integer aPolicy22AgentIndex)
        throws SnmpStatusException  {
        SnmpOid oid = new SnmpOid();
        try {
            SnmpInt.appendToOid(new SnmpInt(aSsoServerRealmIndex).toOid(), oid);
            SnmpInt.appendToOid(new SnmpInt(aPolicy22AgentIndex).toOid(), oid);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.noSuchInstance);
        }
        return oid;
     ...
     *         "SsoServerPolicy22AgentEntry" conceptual row.
     * 
     * Note that when using standard metadata,
     * the returned object must implement the "SsoServerPolicy22AgentEntryMBean"
     * interface.
    ...
    public Object createSsoServerPolicy22AgentEntryMBean(SnmpMibSubRequest req,
                SnmpOid rowOid, int depth, ObjectName entryObjName,
                SnmpMibTable meta, Integer  aSsoServerRealmIndex, Integer  aPolicy22AgentIndex)
            throws SnmpStatusException  {
        ...
        // interface.
        //
        SsoServerPolicy22AgentEntry entry = new SsoServerPolicy22AgentEntry(theMib);
        entry.SsoServerRealmIndex = aSsoServerRealmIndex;
        entry.Policy22AgentIndex = aPolicy22AgentIndex;
        return entry;
    }
