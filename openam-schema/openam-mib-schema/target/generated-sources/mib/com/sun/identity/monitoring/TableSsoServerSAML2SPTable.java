 ...
 */
public class TableSsoServerSAML2SPTable extends SnmpTableSupport implements Serializable {

    /**
     * Constructor for the table. Initialize metadata for "TableSsoServerSAML2SPTable".
     ...
     */
    public TableSsoServerSAML2SPTable(SnmpMib myMib) {
        super(myMib);
    }
     ...
     */
    public TableSsoServerSAML2SPTable(SnmpMib myMib, MBeanServer server) {
        this(myMib);
        this.server = server;
    }
    ...
    public void createNewEntry(SnmpMibSubRequest req, SnmpOid rowOid,
        ...
        try  {
            ...
            // the returned object must implement the "SsoServerSAML2SPEntryMBean"
            // interface.
            //
            final Object entry =
                 createSsoServerSAML2SPEntryMBean(req, rowOid, depth, objname, meta,
                    oid0.toInteger(),
                    oid1.toInteger());
            if (server != null) {
    ...
    protected SnmpMibTable getRegisteredTableMeta(SnmpMib mib)  {
        return mib.getRegisteredTableMeta("SsoServerSAML2SPTable");
    }
    ...
    public synchronized void addEntry(SsoServerSAML2SPEntryMBean entry)
        throws SnmpStatusException {
    ...
    public synchronized void addEntry(SsoServerSAML2SPEntryMBean entry, ObjectName name)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
    ...
    public synchronized SsoServerSAML2SPEntryMBean[] getEntries() {
        Object[] array = getBasicEntries();
        SsoServerSAML2SPEntryMBean[] result = new SsoServerSAML2SPEntryMBean[array.length];
    ...
    public void removeEntry(SsoServerSAML2SPEntryMBean entry)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
    ...
    public ObjectName buildNameFromIndex(SnmpIndex index)
        ...
        try  {
            ...
            String _keySAML2SPIndex = oid.toInteger().toString();
            return new ObjectName("TableSsoServerSAML2SPTable:name=com.sun.identity.monitoring.SsoServerSAML2SPEntry" + ",SsoServerRealmIndex=" + _keySsoServerRealmIndex + ",SAML2SPIndex=" + _keySAML2SPIndex);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.snmpRspWrongValue);
        } catch(Exception e) {
            throw new SnmpStatusException(e.getMessage());
     ...
     */
    public SnmpIndex buildSnmpIndex(SsoServerSAML2SPEntryMBean entry)
        throws SnmpStatusException {
        SnmpOid[] oids = new SnmpOid[2];
        SnmpValue val = null;
        val = new SnmpInt(entry.getSsoServerRealmIndex());
        oids[0] = val.toOid();
        val = new SnmpInt(entry.getSAML2SPIndex());
        oids[1] = val.toOid();
     ...
     * Build index for "SsoServerSAML2SPEntry".
     */
    public SnmpOid buildOidFromIndexVal(Integer aSsoServerRealmIndex, Integer aSAML2SPIndex)
        throws SnmpStatusException  {
        SnmpOid oid = new SnmpOid();
        try {
            SnmpInt.appendToOid(new SnmpInt(aSsoServerRealmIndex).toOid(), oid);
            SnmpInt.appendToOid(new SnmpInt(aSAML2SPIndex).toOid(), oid);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.noSuchInstance);
        }
        return oid;
     ...
     *         "SsoServerSAML2SPEntry" conceptual row.
     * 
     * Note that when using standard metadata,
     * the returned object must implement the "SsoServerSAML2SPEntryMBean"
     * interface.
    ...
    public Object createSsoServerSAML2SPEntryMBean(SnmpMibSubRequest req,
                SnmpOid rowOid, int depth, ObjectName entryObjName,
                SnmpMibTable meta, Integer  aSsoServerRealmIndex, Integer  aSAML2SPIndex)
            throws SnmpStatusException  {
        ...
        // interface.
        //
        SsoServerSAML2SPEntry entry = new SsoServerSAML2SPEntry(theMib);
        entry.SsoServerRealmIndex = aSsoServerRealmIndex;
        entry.SAML2SPIndex = aSAML2SPIndex;
        return entry;
    }
