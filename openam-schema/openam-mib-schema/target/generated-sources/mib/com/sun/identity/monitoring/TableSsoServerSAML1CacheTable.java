 ...
 * The group is defined with the following oid: 1.3.6.1.4.1.42.2.230.3.1.1.2.1.16.1.
 */
public class TableSsoServerSAML1CacheTable extends SnmpTableSupport implements Serializable {

    /**
     * Constructor for the table. Initialize metadata for "TableSsoServerSAML1CacheTable".
     ...
     */
    public TableSsoServerSAML1CacheTable(SnmpMib myMib) {
        super(myMib);
    }
     ...
     */
    public TableSsoServerSAML1CacheTable(SnmpMib myMib, MBeanServer server) {
        this(myMib);
        this.server = server;
    }
    ...
    public void createNewEntry(SnmpMibSubRequest req, SnmpOid rowOid,
        ...
        try  {
            ...
            // the returned object must implement the "SsoServerSAML1CacheEntryMBean"
            // interface.
            //
            final Object entry =
                 createSsoServerSAML1CacheEntryMBean(req, rowOid, depth, objname, meta,
                    oid0.toInteger());
            if (server != null) {
    ...
    protected SnmpMibTable getRegisteredTableMeta(SnmpMib mib)  {
        return mib.getRegisteredTableMeta("SsoServerSAML1CacheTable");
    }
    ...
    public synchronized void addEntry(SsoServerSAML1CacheEntryMBean entry)
        throws SnmpStatusException {
    ...
    public synchronized void addEntry(SsoServerSAML1CacheEntryMBean entry, ObjectName name)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
    ...
    public synchronized SsoServerSAML1CacheEntryMBean[] getEntries() {
        Object[] array = getBasicEntries();
        SsoServerSAML1CacheEntryMBean[] result = new SsoServerSAML1CacheEntryMBean[array.length];
    ...
    public void removeEntry(SsoServerSAML1CacheEntryMBean entry)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
    ...
    public ObjectName buildNameFromIndex(SnmpIndex index)
        ...
        try  {
            ...
            String _keySAML1CacheIndex = oid.toInteger().toString();
            return new ObjectName("TableSsoServerSAML1CacheTable:name=com.sun.identity.monitoring.SsoServerSAML1CacheEntry" + ",SAML1CacheIndex=" + _keySAML1CacheIndex);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.snmpRspWrongValue);
        } catch(Exception e) {
            throw new SnmpStatusException(e.getMessage());
     ...
     */
    public SnmpIndex buildSnmpIndex(SsoServerSAML1CacheEntryMBean entry)
        throws SnmpStatusException {
        SnmpOid[] oids = new SnmpOid[1];
        SnmpValue val = null;
        val = new SnmpInt(entry.getSAML1CacheIndex());
        oids[0] = val.toOid();
     ...
     * Build index for "SsoServerSAML1CacheEntry".
     */
    public SnmpOid buildOidFromIndexVal(Integer aSAML1CacheIndex)
        throws SnmpStatusException  {
        SnmpOid oid = new SnmpOid();
        try {
            SnmpInt.appendToOid(new SnmpInt(aSAML1CacheIndex).toOid(), oid);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.noSuchInstance);
        }
        return oid;
     ...
     *         "SsoServerSAML1CacheEntry" conceptual row.
     * 
     * Note that when using standard metadata,
     * the returned object must implement the "SsoServerSAML1CacheEntryMBean"
     * interface.
    ...
    public Object createSsoServerSAML1CacheEntryMBean(SnmpMibSubRequest req,
                SnmpOid rowOid, int depth, ObjectName entryObjName,
                SnmpMibTable meta, Integer  aSAML1CacheIndex)
            throws SnmpStatusException  {
        ...
        // interface.
        //
        SsoServerSAML1CacheEntry entry = new SsoServerSAML1CacheEntry(theMib);
        entry.SAML1CacheIndex = aSAML1CacheIndex;
        return entry;
    }
