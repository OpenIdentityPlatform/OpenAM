 ...
 * The group is defined with the following oid: 1.3.6.1.4.1.42.2.230.3.1.1.2.1.22.1.
 */
public class TableSsoServerWSSAgentsWSCAgentTable extends SnmpTableSupport implements Serializable {

    /**
     * Constructor for the table. Initialize metadata for "TableSsoServerWSSAgentsWSCAgentTable".
     ...
     */
    public TableSsoServerWSSAgentsWSCAgentTable(SnmpMib myMib) {
        super(myMib);
    }
     ...
     */
    public TableSsoServerWSSAgentsWSCAgentTable(SnmpMib myMib, MBeanServer server) {
        this(myMib);
        this.server = server;
    }
    ...
    public void createNewEntry(SnmpMibSubRequest req, SnmpOid rowOid,
        ...
        try  {
            ...
            // the returned object must implement the "SsoServerWSSAgentsWSCAgentEntryMBean"
            // interface.
            //
            final Object entry =
                 createSsoServerWSSAgentsWSCAgentEntryMBean(req, rowOid, depth, objname, meta,
                    oid0.toInteger(),
                    oid1.toInteger());
            if (server != null) {
    ...
    protected SnmpMibTable getRegisteredTableMeta(SnmpMib mib)  {
        return mib.getRegisteredTableMeta("SsoServerWSSAgentsWSCAgentTable");
    }
    ...
    public synchronized void addEntry(SsoServerWSSAgentsWSCAgentEntryMBean entry)
        throws SnmpStatusException {
    ...
    public synchronized void addEntry(SsoServerWSSAgentsWSCAgentEntryMBean entry, ObjectName name)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
    ...
    public synchronized SsoServerWSSAgentsWSCAgentEntryMBean[] getEntries() {
        Object[] array = getBasicEntries();
        SsoServerWSSAgentsWSCAgentEntryMBean[] result = new SsoServerWSSAgentsWSCAgentEntryMBean[array.length];
    ...
    public void removeEntry(SsoServerWSSAgentsWSCAgentEntryMBean entry)
        throws SnmpStatusException {
        SnmpIndex index = buildSnmpIndex(entry);
    ...
    public ObjectName buildNameFromIndex(SnmpIndex index)
        ...
        try  {
            ...
            String _keyWssAgentsWSCAgentIndex = oid.toInteger().toString();
            return new ObjectName("TableSsoServerWSSAgentsWSCAgentTable:name=com.sun.identity.monitoring.SsoServerWSSAgentsWSCAgentEntry" + ",SsoServerRealmIndex=" + _keySsoServerRealmIndex + ",WssAgentsWSCAgentIndex=" + _keyWssAgentsWSCAgentIndex);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.snmpRspWrongValue);
        } catch(Exception e) {
            throw new SnmpStatusException(e.getMessage());
     ...
     */
    public SnmpIndex buildSnmpIndex(SsoServerWSSAgentsWSCAgentEntryMBean entry)
        throws SnmpStatusException {
        SnmpOid[] oids = new SnmpOid[2];
        SnmpValue val = null;
        val = new SnmpInt(entry.getSsoServerRealmIndex());
        oids[0] = val.toOid();
        val = new SnmpInt(entry.getWssAgentsWSCAgentIndex());
        oids[1] = val.toOid();
     ...
     * Build index for "SsoServerWSSAgentsWSCAgentEntry".
     */
    public SnmpOid buildOidFromIndexVal(Integer aSsoServerRealmIndex, Integer aWssAgentsWSCAgentIndex)
        throws SnmpStatusException  {
        SnmpOid oid = new SnmpOid();
        try {
            SnmpInt.appendToOid(new SnmpInt(aSsoServerRealmIndex).toOid(), oid);
            SnmpInt.appendToOid(new SnmpInt(aWssAgentsWSCAgentIndex).toOid(), oid);
        } catch(ArrayIndexOutOfBoundsException e) {
            throw new SnmpStatusException(SnmpStatusException.noSuchInstance);
        }
        return oid;
     ...
     *         "SsoServerWSSAgentsWSCAgentEntry" conceptual row.
     * 
     * Note that when using standard metadata,
     * the returned object must implement the "SsoServerWSSAgentsWSCAgentEntryMBean"
     * interface.
    ...
    public Object createSsoServerWSSAgentsWSCAgentEntryMBean(SnmpMibSubRequest req,
                SnmpOid rowOid, int depth, ObjectName entryObjName,
                SnmpMibTable meta, Integer  aSsoServerRealmIndex, Integer  aWssAgentsWSCAgentIndex)
            throws SnmpStatusException  {
        ...
        // interface.
        //
        SsoServerWSSAgentsWSCAgentEntry entry = new SsoServerWSSAgentsWSCAgentEntry(theMib);
        entry.SsoServerRealmIndex = aSsoServerRealmIndex;
        entry.WssAgentsWSCAgentIndex = aWssAgentsWSCAgentIndex;
        return entry;
    }
