 ...
 *
 * $Id: FamilySettings.fx,v 1.2 2009/06/11 05:29:43 superpat7 Exp $
 */

package c1democlient;

import c1democlient.Constants;
import c1democlient.FamilySettings;
...
import c1democlient.PhoneSettings;
...
public function doFamilySettings(): Void {
    // Have to load data before we create the UI elements
    loadPhones(Main.account.accountNumber);
}

function loadPhones(accountNumber:String): Void {
    println("Call Accounts Restful Web Service via OAuth...");
    ...
    var oauthRequest: OauthRequest = OauthRequest {
        ...
        request: HttpRequest {
            ...
            onResponseCode: function(responseCode:Integer) {
                ...
                if (responseCode != 200) {
                    ErrorMessage.doError("Error loading phone settings", Home.doHome );
                }
            }
                    ...
                    for ( phone in phones ) {
                        var textBox = TextButton {
                            ...
                            action: function(): Void {
                                PhoneSettings.doPhoneSettings(phone, doFamilySettings);
                            }
                        }
                        var number = Text {
                        ...
                        var minutes = Text {
                            content: phone.allocatedMinutes;
                            textOrigin: TextOrigin.TOP;
                        ...
                        insert minutes into tableContent;
                    }
                    Main.goto(FamilySettings{
...
public class FamilySettings extends CustomNode {
    ...
    public override function create():Node {
        ...
        var vbox = VBox {
            content: [
                ...
                Text {
                    font: Constants.arialBold12
                    content: "Allocate minutes from your monthly pool"
                    wrappingWidth: 210
                }
                Text {
                    font: Constants.arialRegular12
                    content: "You can view and edit settings for each phone "
