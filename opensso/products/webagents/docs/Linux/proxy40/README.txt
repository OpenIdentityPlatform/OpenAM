...
%% Contents:
	...
	%% 2.6 Download SJS ProxY Server 4.0 Header files 
        %% 2.7 Obtaining OpenAM Agents Common Installer libraries
    %% 3. Building the workspace

%% 1. Build requirements

The OpenAM - Web Agents workspace uses GNU Make as the build tool. 
...
2.6.1 Open the web page: http://www.sun.com/download. Choose Proxy Server.
2.6.2 Download Proxy Server 4.0 install bits.
...
Building SJS Proxy Server 4.0 agent:

     - ant proxy40 : builds SJS Proxy Server 4 agent. C code compiled in optimized mode.
     - ant proxy40 -Dbuild.debug=full     : builds SJS Proxy Server 4 agent. C code compiled in debug mode.
     - ant proxy40 -Dbuild.debug=optimize : builds SJS Proxy Server 4 agent. C code compiled in optimized mode.
...
name is in this format: proxy_v40_<OS>_agent.zip.

    - <OS> : SunOS, Linux, WINNT, SunOS_x86

Example: proxy_v40_Linux_agent.zip
