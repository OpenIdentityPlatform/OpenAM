...
extern "C"
am_status_t am_worker_cleanup(void) {
    Log::ModuleId logID = Log::addModule("am_worker_cleanup()");
    am_status_t status = AM_SUCCESS;
    ScopeLock myLock(initLock);
    if (initialized) {
        try {
            status = Connection::shutdown_in_child_process();
            Log::log(logID, Log::LOG_DEBUG, "am_worker_cleanup(): succeeded");
        } catch (const NSPRException& exc) {
            Log::log(logID, Log::LOG_ERROR, exc);
            status = AM_NSPR_ERROR;
        } catch (const std::exception& exc) {
            ...
            Log::log(logID, Log::LOG_ERROR, "am_worker_cleanup(): Unknown exception encountered");
            status = AM_FAILURE;
        }
    }
    return status;
