 ...
 *
 * $Id: IdServicesProviderImpl.java,v 1.3 2008/06/25 05:43:33 qcheng Exp $
 *
 */
 ...
 */
public class IdServicesProviderImpl implements IdServicesProvider {
    ...
    static {
        ...
        if (cachingEnabled) {
            idServices = IdCachedServicesImpl.getInstance();
            if (debug.messageEnabled()) {
                debug.message("IdServicesProviderImpl.static{} - "
                        + "Caching Mode: " + cachingEnabled + "Using "
                        + "implementation Class IdCachedServicesImpl");
            }
        } else {
            idServices = IdServicesImpl.getInstance();
            if (debug.messageEnabled()) {
                debug.message("IdServicesProviderImpl.static{} - "
                        + "Caching Mode: " + cachingEnabled + "Using "
                        + "implementation Class IdServicesImpl");
            }
        }
    }
