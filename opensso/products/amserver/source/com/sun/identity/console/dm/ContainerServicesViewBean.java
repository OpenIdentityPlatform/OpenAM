 ...
 *
 * $Id: ContainerServicesViewBean.java,v 1.2 2008/06/25 05:42:53 qcheng Exp $
 *
 */
...
public class ContainerServicesViewBean
    extends ContainerPropertiesBase
{
    public static final String DEFAULT_DISPLAY_URL =
	"/console/dm/ContainerServices.jsp";
     ...
     */
    public ContainerServicesViewBean() {
	...
	return new ContainerModelImpl(req, getPageSessionAttributes());
    }
    ...
    private  void createTableModel() {
        tblModel = new CCActionTableModel(
            getClass().getClassLoader().getResourceAsStream(
                "com/sun/identity/console/tblDMServices.xml"));
        tblModel.setTitleLabel("label.items");
        tblModel.setActionValue(TBL_BUTTON_ADD, "table.dm.button.add.service");
        tblModel.setActionValue(TBL_BUTTON_DELETE, 
	    "table.dm.button.remove.service");
	...
	if ((services != null) && !services.isEmpty()) {
	    ...
	    for (Iterator iter = services.keySet().iterator(); 
	        iter.hasNext();) 
	    {
		if (firstEntry) {
		    firstEntry = false;
    ...
    public void handleTblButtonAddRequest(RequestInvocationEvent event) {
        ...
        SelectServicesViewBean vb = (SelectServicesViewBean)getViewBean(
	    SelectServicesViewBean.class);
        unlockPageTrail();
	passPgSessionMap(vb);
