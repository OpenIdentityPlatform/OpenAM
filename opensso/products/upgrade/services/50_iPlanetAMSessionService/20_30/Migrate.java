 ...
 *
 * $Id: Migrate.java,v 1.2 2008/06/25 05:53:50 qcheng Exp $
 *
 */
...
/**
 * Updates <code>iPlanetAMSessionService</code> service schema.
...
public class Migrate implements MigrateTasks {
    ...
    final static String SERVICE_DIR = "50_iPlanetAMSessionService/20_30";
    final static String[] SCHEMA_FILE_LIST =
            {"amSession_add.xml", "amSession_mod.xml"};
    final static String LDIF_FILE = "amSession_schema.ldif";
    ...
    public boolean migrateService() {
        ...
        try {
            ...
            for (int i = 0; i < numFiles; i++) {
                fileName =
                        UpgradeUtils.getAbsolutePath(SERVICE_DIR,
                        SCHEMA_FILE_LIST[i]);
                fileList[i] = fileName;
            }
