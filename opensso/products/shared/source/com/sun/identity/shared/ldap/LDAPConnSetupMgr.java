...
class LDAPConnSetupMgr implements Cloneable, java.io.Serializable {
     ...
     */
    private transient Socket m_socket = null;
    
    /**
     * Original, underlying socket to the server, see layerSocket()
     */
    private transient Socket m_origSocket = null;
    ...
    LDAPConnSetupMgr(String[] hosts, int[] ports, LDAPSocketFactory factory)  throws LDAPException{        
        ...
        for (int i=0; i < hosts.length; i++) {
            ...
            try { 
                ...
                throw new LDAPException("Invalid host:port " + hosts[i]+":" +
                    ports[i], LDAPException.PARAM_ERROR);                
            }
        }
        m_factory = factory;
    }
    ...
    */
     ...
     */
    protected synchronized void invalidateConnection() {
        if (m_socket != null) {
     ...
     */    
    public synchronized Socket layerSocket(LDAPTLSSocketFactory factory) throws
        LDAPException{
    ...
    public synchronized int  getConnSetupDelay() {
     ...
     */
    public synchronized void setConnSetupDelay(int delay) {
     ...
     */
    public synchronized boolean isUserDisconnected() {
        return (m_dsIdx >=0 &&
                m_dsList[m_dsIdx].connSetupStatus == DISCONNECTED);
    }
    ...
    private void connectServer(int idx) {    
        ...
        try {
            ...
            if (!entry.url.isSecure()) {                
                ...
                if (factory == null) {
                    throw new LDAPException(
                        "Can not connect, no socket factory " + entry.url,
                        LDAPException.OTHER);
                }
    ...
    *  (4) connection setup failed/connection lost
     ...
     */
    public synchronized boolean breakConnection() {
        try {                
            m_socket.close();
            return true;
        }
    ...
    public Object clone() {
        try {
            ...
            for (int i=0; i<m_dsList.length; i++) {
                ...
                cloneMgr.m_dsList[i] = new ServerEntry(e.url, e.connSetupStatus);
            }
            return  cloneMgr;
        }
