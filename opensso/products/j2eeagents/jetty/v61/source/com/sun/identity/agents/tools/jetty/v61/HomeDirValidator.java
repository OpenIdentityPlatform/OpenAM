...
public class HomeDirValidator extends ValidatorBase implements IConstants,
    ...
    public ValidationResult isHomeDirValid(
        ...
        if ((jettyHomeDir != null) && (jettyHomeDir.trim().length() >= 0)) {
            ...
            String jettyJarFile = jettyHomeDir + STR_FORWARD_SLASH + 
                    JETTY_START_JAR;
            String webAppDir =jettyHomeDir + STR_FORWARD_SLASH + 
                    JETTY_WEB_APP_DIR;
            if (FileUtils.isFileValid(jettyJarFile) && 
                    canUpdateJettyConfigFile(jettyHomeDir)) {
                returnMessage = LocalizedMessage.get(VA_MSG_JETTY_VAL_HOME,
                        STR_JETTY_GROUP, new Object[] { jettyHomeDir });
                validRes = ValidationResultStatus.STATUS_SUCCESS;
                state.put(STR_KEY_JETTY_START_JAR_PATH, jettyJarFile);
                Debug.log("HomeDirValidator: Added Jetty home to state --> " + 
                        jettyHomeDir); 
                state.put(STR_KEY_JETTY_INST_DEPLOY_DIR, webAppDir);
                Debug.log("HomeDirValidator: Added Jetty webapp dir to state " +
                        "--> " + webAppDir);
            }
        }
        if (validRes.getIntValue() == ValidationResultStatus.INT_STATUS_FAILED) 
        {
            returnMessage = LocalizedMessage.get(VA_WRN_JETTY_IN_VAL_HOME,
                    STR_JETTY_GROUP, new Object[] { jettyHomeDir });
        }
        Debug.log("HomeDirValidator : Is Jetty Home directory " + jettyHomeDir + 
    ...
    public void initializeValidatorMap() throws InstallException {
        ...
        try {
            getValidatorMap()
                .put(
                "VALID_JETTY_HOME_DIR",
                this.getClass().getMethod(
                    "isHomeDirValid",
                    paramObjs));
        } catch (NoSuchMethodException nsme) {
            Debug.log("HomeDirValidator: NoSuchMethodException " + 
                    "thrown while loading method :", nsme);
            throw new InstallException(
                LocalizedMessage.get(LOC_VA_ERR_VAL_METHOD_NOT_FOUND),
                nsme);
        } catch (SecurityException se) {
            Debug.log("HomeDirValidator: SecurityException thrown " + 
                    "while loading method :", se);
            throw new InstallException(
                LocalizedMessage.get(LOC_VA_ERR_VAL_METHOD_NOT_FOUND),
                se);
        } catch (Exception ex) {
            Debug.log("HomeDirValidator: Exception thrown while "+ 
                    "loading method :", ex);
            throw new InstallException(
                LocalizedMessage.get(LOC_VA_ERR_VAL_METHOD_NOT_FOUND), ex);
        }
    }

    protected boolean canUpdateJettyConfigFile(String jettyHomeDir) {
        boolean result = false;
        String jettyConfFile = jettyHomeDir + STR_FORWARD_SLASH + 
                STR_ETC_DIRECTORY + STR_FORWARD_SLASH + STR_SERVER_XML;
