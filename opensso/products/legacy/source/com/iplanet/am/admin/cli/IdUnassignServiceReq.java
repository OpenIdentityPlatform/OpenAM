 ...
 *
 * $Id: IdUnassignServiceReq.java,v 1.2 2008/06/25 05:52:29 qcheng Exp $
 *
 */
...
class IdUnassignServiceReq extends AdminReq {
    ...
    /**
     * Constructs a new IdUnassignServiceReq.
     *
     * @param targetDN the parent Realm DN. 
     */        
    IdUnassignServiceReq(String targetDN) {
        super(targetDN);
        realmPath = targetDN;
    }
    ...
    public String toString() {
        ...
        prnWriter.println(AdminReq.bundle.getString("requestdescription131") +
            " " + targetDN);
        prnWriter.flush();
        ...
        throws AdminException
    {
        AdminReq.writer.println(bundle.getString("identity") + "\n" +
            bundle.getString("rmServiceId") + " " + serviceName + " " +
            bundle.getString("fromIdentity") + " " +
            idName + " " +
            bundle.getString("of") + " " +
            idType.toString() + " " +
            bundle.getString("inrealm") + " " + realmPath);

        String[] args = {serviceName, idType.toString(), idName,
            realmPath};
        ...
        try {
            ...
            doLog(args, AdminUtils.UNASSIGN_SERVICE_IDENTITY_ATTEMPT);
            ai2use.unassignService(serviceName);
            doLog(args, AdminUtils.UNASSIGN_SERVICE_IDENTITY);
