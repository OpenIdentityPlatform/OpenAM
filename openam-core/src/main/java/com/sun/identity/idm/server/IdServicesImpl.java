   ...
   public boolean authenticate(String orgName, Callback[] credentials, IdType idType)
               ...
               for (IdRepo idRepo : cPlugins) {
                   if (idRepo.getClass().getName().equals(IdConstants.SPECIAL_PLUGIN)) {
                       if (idRepo.authenticate(credentials)) {
                           if (DEBUG.messageEnabled()) {
                               DEBUG.message(
                                       "IdServicesImpl.authenticate: AuthN success using special repo {} user: {}",
                                       idRepo.getClass().getName(), name);
                           }
                           return true;
                       } else {
                           // Invalid password used for internal user
                           DEBUG.error("IdServicesImpl.authenticate: AuthN failed using special repo {}, user: {}",
                                   idRepo.getClass().getName(), name);
                           return false;
                       }
       ...
       for (Iterator items = cPlugins.iterator(); items.hasNext();) {
           ...
           if (idRepo.supportsAuthentication()) {
               if (DEBUG.messageEnabled()) {
                   DEBUG.message("IdServicesImpl.authenticate: AuthN to {} in org: {}",
                       idRepo.getClass().getName(), orgName);
               }
               try {
                   if (idRepo.authenticate(credentials)) {
                       // Successfully authenticated
                       if (DEBUG.messageEnabled()) {
                           DEBUG.message("IdServicesImpl.authenticate: AuthN success for {}",
