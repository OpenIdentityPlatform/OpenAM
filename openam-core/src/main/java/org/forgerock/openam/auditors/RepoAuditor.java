...
public class RepoAuditor {
     ...
     */
    @Inject
    public RepoAuditor(@Named("amIdm") Debug debug, AuditEventPublisher auditEventPublisher,
    ...
    protected AMConfigAuditEventBuilder getBaseBuilder() {
        return auditEventFactory.configEvent(realm)
                ...
                .eventName(EventName.AM_USER_CHANGE);
    }
    ...
    protected boolean shouldAudit(ConfigOperation operation) {
        return auditEventPublisher.isAuditing(realm, ACTIVITY_TOPIC, EventName.AM_USER_CHANGE)
                    && isAudited(operation);
    }
