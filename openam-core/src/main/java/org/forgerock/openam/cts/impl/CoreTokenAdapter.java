...
public class CoreTokenAdapter {
     ...
     */
    public ResultHandler<Token> create(Token token) throws CoreTokenException {
        applyBlobStrategy(token);
        debug("Create: queued {0} Token {1}\n{2}", token.getType(), token.getTokenId(), token);
        final ResultHandler<Token> createHandler = handlerFactory.getCreateHandler();
     ...
     */
    public ResultHandler<Token> updateOrCreate(Token token) throws CoreTokenException {
        applyBlobStrategy(token);
        debug("UpdateOrCreate: queued {0} Token {1}\n{2}", token.getType(), token.getTokenId(), token);
        final ResultHandler<Token> updateHandler = handlerFactory.getUpdateHandler();
     ...
     */
    public ResultHandler<String> delete(String tokenId) throws CoreTokenException {
        debug("Delete: queued delete {0}", tokenId);
        final ResultHandler<String> deleteHandler = handlerFactory.getDeleteHandler();
