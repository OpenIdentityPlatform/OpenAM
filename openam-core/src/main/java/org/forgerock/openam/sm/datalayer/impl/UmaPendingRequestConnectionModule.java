...
public class UmaPendingRequestConnectionModule extends LdapDataLayerConnectionModule {

    @Override
    protected Class<? extends LdapDataLayerConfiguration> getLdapConfigurationType() {
        return UmaPendingRequestDataLayerConfiguration.class;
    }
    ...
    protected void configureDataStore(PrivateBinder binder) {
        ...
        binder.bind(storeKey).to(UmaPendingRequestTokenDataStore.class).in(Singleton.class);
        binder.expose(storeKey);
    }

    private static final class UmaPendingRequestTokenDataStore extends TokenDataStore<UmaPendingRequest> {
         ...
         */
        @Inject
        public UmaPendingRequestTokenDataStore(JavaBeanAdapterFactory adapterFactory, TaskExecutor taskExecutor,
                TaskFactory taskFactory) {
            super((JavaBeanAdapter<UmaPendingRequest>) adapterFactory.create(UmaPendingRequest.class), taskExecutor, taskFactory);
        }
    }
}
