...
public class J2EEAgentConfigValidator extends AgentBase implements 
        ...
        if (loadConfig(fName) && validateBootProperties()) {
            ...
            if (ssoToken != null) {
                ...
                toolOutWriter.printError("agt-auth-to-svr-fail" ,
                    new String[] {SystemProperties.get(J2EE_AGENT_USER_NAME)});
            }
        } else {
    ...
    private String getAppSSOTokenStr() {
        ...
        try {
            ...
            SystemProperties.initializeProperties(ENC_PWD_PROPERTY,
                SystemProperties.get(ENC_PWD_PROPERTY));
            String pwd = Crypt.decode(encPwd);
            String uName = SystemProperties.get(J2EE_AGENT_USER_NAME);
            
            String authURL=getBaseUrlStr(SystemProperties.get(AM_NAMING_URL)) +
    ...
    public void processAgents(String token) {
        try {
            String restUrl =
                getBaseUrlStr(SystemProperties.get(AM_NAMING_URL)) +
                "/identity/read";
            String data4 =
