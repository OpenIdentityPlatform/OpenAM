...
public class SmsRequestHandler implements RequestHandler, SMSObjectListener, ServiceListener {
    ...
    private void addSpecialCaseRoutes(PrivilegedAction<SSOToken> adminTokenAction) throws SMSException, SSOException {
        ...
        addSitesHandler();
        addResourcelessHandlers(adminTokenAction);
    }

    private void addResourcelessHandlers(PrivilegedAction<SSOToken> adminTokenAction) throws SSOException, SMSException {
        ServiceSchemaManager ssm = new ServiceSchemaManager(ISAuthConstants.AUTH_SERVICE_NAME, adminTokenAction.run());

        // global-config/authentication -> schema + template only
        if (schemaType == SchemaType.GLOBAL) {
            ResourcelessSmsHandler globalAuth = new ResourcelessSmsHandler(ssm.getGlobalSchema(), SchemaType.GLOBAL,
                    null, null, false, new SmsJsonConverter(ssm.getGlobalSchema()), debug);

            routeTree.addRoute(EQUALS, "/authentication", globalAuth);

            // global-config/services
            getServiceInstanceRouter().addRoute(EQUALS, "", InjectorHolder.getInstance(ServicesGlobalSmsHandler.class));
        }

        // realm-config/authentication -> schema + template only
        if (schemaType == SchemaType.ORGANIZATION) {
            ResourcelessSmsHandler orgAuth = new ResourcelessSmsHandler(ssm.getOrganizationSchema(), SchemaType.ORGANIZATION,
                    null, null, false, new SmsJsonConverter(ssm.getOrganizationSchema()), debug);

            routeTree.addRoute(EQUALS, "/authentication", orgAuth);

            // realm-config/services
            getServiceInstanceRouter().addRoute(EQUALS, "", InjectorHolder.getInstance(ServicesRealmSmsHandler.class));
        }
