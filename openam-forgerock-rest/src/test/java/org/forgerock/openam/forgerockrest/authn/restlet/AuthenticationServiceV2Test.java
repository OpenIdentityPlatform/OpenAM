...
public class AuthenticationServiceV2Test {
    ...
    public void shouldFailAuthenticationWithUnsupportedMediaTypeMessage() throws IOException, JSONException {
        ...
        assertEquals(jsonRep.getJsonObject().has("code"), true);
        assertEquals(jsonRep.getJsonObject().get("code"), 415);
        assertEquals(jsonRep.getJsonObject().has("reason"), true);
        assertEquals(jsonRep.getJsonObject().get("reason"), "Unsupported Media Type");
        assertEquals(jsonRep.getJsonObject().has("message"), true);
        assertEquals(jsonRep.getJsonObject().get("message"), "Unsupported Media Type");
    }
    ...
    public void shouldReturnUnauthorizedCodeWithJsonFailureMessage() throws IOException, JSONException {
        ...
        assertEquals(jsonRep.getJsonObject().has("failure"), true);
        assertEquals(jsonRep.getJsonObject().get("failure"), true);
        assertEquals(jsonRep.getJsonObject().has("reason"), true);
        assertEquals(jsonRep.getJsonObject().get("reason"), "http-auth-failed");
        assertEquals(jsonRep.getJsonObject().has("authId"), true);
        assertEquals(jsonRep.getJsonObject().get("authId"), "12345");
    }
    ...
    public void shouldThrowResourceExceptionContainingUnauthorizedCodeWithJsonErrorMessage()
        ...
        JsonRepresentation jsonRep = new JsonRepresentation(
                new RestStatusService().getRepresentation(exception.getStatus(), null, null));
        assertEquals(jsonRep.getJsonObject().has("code"), true);
        assertEquals(jsonRep.getJsonObject().get("code"), 401);
        assertEquals(jsonRep.getJsonObject().has("reason"), true);
        assertEquals(jsonRep.getJsonObject().get("reason"), "Unauthorized");
        assertEquals(jsonRep.getJsonObject().has("message"), true);
        assertEquals(jsonRep.getJsonObject().get("message"), "Invalid Password!!");
        assertEquals(jsonRep.getJsonObject().has("detail"), true);
        assertEquals(jsonRep.getJsonObject().getJSONObject("detail").has("failureUrl"), true);
        assertEquals(jsonRep.getJsonObject().getJSONObject("detail").get("failureUrl"), "http://localhost:8080");
    }
}
