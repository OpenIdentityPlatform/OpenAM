...
public class AuthenticationServiceV1Test {
    ...
    public void shouldFailAuthenticationWithUnsupportedMediaTypeMessage() throws IOException, JSONException {
        ...
        assertEquals(jsonRep.getJsonObject().has("errorMessage"), true);
        assertEquals(jsonRep.getJsonObject().get("errorMessage"), "Unsupported Media Type");
    }
    ...
    public void shouldReturnUnauthorizedCodeWithJsonErrorMessage() throws IOException, JSONException {
        ...
        JsonRepresentation jsonRep = new JsonRepresentation(rep);
        assertEquals(jsonRep.getJsonObject().has("errorMessage"), true);
        assertEquals(jsonRep.getJsonObject().get("errorMessage"), "Invalid Password!!");
        assertEquals(jsonRep.getJsonObject().has("failureUrl"), true);
        assertEquals(jsonRep.getJsonObject().get("failureUrl"), "http://localhost:8080");
    }
}
