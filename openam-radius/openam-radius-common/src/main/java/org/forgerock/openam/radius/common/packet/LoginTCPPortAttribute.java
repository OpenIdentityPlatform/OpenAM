...
public class LoginTCPPortAttribute extends Attribute {
    ...
    /**
     * The TCP port with which the user is to be connected, when the Login-Service Attribute is also present.
     */
    private int port = 0;
     ...
     */
    public LoginTCPPortAttribute(byte[] octets) {
        super(octets);
        port = OctetUtils.toIntVal(octets);
    }

    /**
     * Constructs a new instance from the TCP port with which the user is to be connected, when the Login-Service
     * Attribute is also present.
     *
     * @param port The TCP port with which the user is to be connected.
     */
    public LoginTCPPortAttribute(int port) {
        super(OctetUtils.toOctets(AttributeType.LOGIN_TCP_PORT, port));
        this.port = port;
    }
     ...
     *
     * @return the TCP port with which the user is to be connected.
     */
    public int getPort() {
        return port;
    }
