...
public class ProxyStateAttribute extends Attribute {
    /**
     * An opaque string to be sent by a proxy server to another server when forwarding an Access-Request and MUST be
     * returned unmodified in the Access-Accept, Access-Reject or Access-Challenge.
     */
     ...
     */
    public ProxyStateAttribute(byte[] octets) {
        super(octets);
        state = new String(octets, 2, octets.length - 2);
    }

    /**
     * Create an instance with the opaque string to be sent by a proxy server to another server when forwarding an
     * Access-Request and MUST be returned unmodified in the Access-Accept, Access-Reject or Access-Challenge.
     *
     * @param state the opaque string
     */
    public ProxyStateAttribute(String state) {
        super(OctetUtils.toOctets(AttributeType.PROXY_STATE, state));
