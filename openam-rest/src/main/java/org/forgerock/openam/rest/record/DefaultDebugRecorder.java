...
public class DefaultDebugRecorder implements DebugRecorder {
    ...
    private PrintWriter getPrintWriterForFile(Record record, String fileName) throws IOException {
        File file = new File(record.getFolderPath() + File.separator + fileName);
        return new PrintWriter(new FileWriter(file, false), true);
    }
    ...
    private void delete(String fileName) throws IOException {
        ...
        if (file.isDirectory()) {
            ...
            if (file.list().length == 0) {
                file.delete();
                debug.message("Directory is deleted : {}.", file.getAbsolutePath());
            } else {
                ...
                for (String temp : files) {
                    ...
                    //recursive delete
                    delete(file.getAbsolutePath()  + File.separator + temp);
                }
            ...
            file.delete();
            debug.message("File is deleted : {}.", file.getAbsolutePath());
        }
