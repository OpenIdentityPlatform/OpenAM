 ...
 *
 * $Id: BEAWeblogicJVMOptionsValidator.java,v 1.1 2008/11/22 02:41:23 ak138937 Exp $
 *
 */
...
public class BEAWeblogicJVMOptionsValidator implements IWebContainerValidate {

    private IToolOutput toolOutWriter;
    
    public BEAWeblogicJVMOptionsValidator() {        
    }
        ...
        if (domainPath != null) {
            ...
            if (domainDir.exists() && domainDir.isDirectory()) {
                ...
                String domainConfig_dir = containerProp.getProperty(
                    WebContainerConstant.JVMOPTIONS_DIR);
                File domainConfigDir = new File(domainDir, domainConfig_dir);
                if (domainConfigDir.exists() && domainConfigDir.isDirectory()) {
                    toolOutWriter.printStatusMsg(true,
                        "webcontainer-domain-bin-dir-check"); 
                    String domainFileName = containerProp.getProperty(
                    ...
                    try {
                        lnRdr = new LineNumberReader(new FileReader(
                            new File(domainConfigDir, domainFileName)));                        
                        String buffer = null;
                    ...
                    toolOutWriter.printError(
                        "webcontainer-domain-bin-dir-invalid");
                    toolOutWriter.printStatusMsg(false,
                        "webcontainer-domain-bin-dir-check"); 
                }
            } else {
