...
/**
 * This class contains the implementation of a config service
 * that can be realized in the Diagnostic tool application.
 *
 */
public class ServerConfigService implements ToolConstants, ConfigConstants, 
    ToolService {
    ...
    /** Creates a new instance of ServerConfigService */
    ...
    public void init(ToolContext tContext) {
        ...
        rb = ResourceBundle.getBundle(SVR_CFG_RESOURCE_BUNDLE);
    }
    ...
    public void processRequest(
        ...
        try {
            ...
            for (Iterator j = commandSet.iterator(); j.hasNext();){
                ...
                ((IServerValidate)validatorMap.get(cmd)).validate(path);
            }
        ...
        try {
            in = new BufferedReader(new FileReader(
                tContext.getApplicationHome() +
                    "/services/server/config/ServerConfiguration"));
            if (in.ready()) {
                String line;
