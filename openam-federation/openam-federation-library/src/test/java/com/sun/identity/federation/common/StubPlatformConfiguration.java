...
/**
 * The platform configuration a multi server site would have, so that the load balancer cookie
 * bounce can be driven from a unit test.
 * <p>
 * {@link FSUtils#needSetLBCookieAndRedirect} starts at {@link FSUtils#requireAddCookie}, which asks
 * {@link com.sun.identity.common.SystemConfigurationUtil} for the platform server list. Without a
 * configuration plugin that lookup ends in a <code>NullPointerException</code> inside
 * <code>initPlatformNaming</code>, the empty server list reads as a single server deployment, and
 * the bounce declines before anything testable happens. One server other than the one under test is
 * all it takes to get past that.
 * <p>
 * Registered through <code>com.sun.identity.plugin.configuration.class</code> in the module's test
 ...
 * first loaded, and surefire runs the whole module in one JVM, so a test that sets it in its own
 * body works only while it runs alone.
 *
 * @see FSUtilsTest
 */
public class StubPlatformConfiguration implements ConfigurationInstance {
     ...
     * it the answer for every getConfigurationInstance call in the module, SAML2 and ID-FF included,
     * and handing those a map holding the server list would look to them like configuration they can
