...
JSONEditor.prototype = {
  ...
  syncModelWithView: function() {
      if(this.editors) {
        for(var i in this.editors) {
          this.editors[i].syncModelWithView();
        }
      }
  },
...
JSONEditor.AbstractEditor = Class.extend({
  ...
  getInlineButton: function(text, icon, title) {
    ...
    return this.theme.getInlineButton(text, icon, title);
  },
...
JSONEditor.defaults.editors.string = JSONEditor.AbstractEditor.extend({
  ...
  syncModelWithView: function() {
      this.handleChangeEvent(this, this.input);
  },
  handleChangeEvent: function(editor, element) {
      // Don't allow changing if this field is a template
      if(editor.schema.template) {
      ...
      // sanitize value
      var sanitized = editor.sanitize(val);
      if(val !== sanitized) {
  ...
  build: function() {
    var self = this, i, inputId = this.theme.getInputId();
    if(!this.options.compact) this.header = this.label = this.theme.getFormInputLabel(this.getTitle());
    if(this.schema.description) this.description = this.theme.getFormInputDescription(this.schema.description);
    if(this.schema.exampleValue) this.placeholder = this.schema.exampleValue;
    ...
    if(this.format) {
      ...
      if(this.format === 'textarea') {
        ...
        this.input = this.theme.getTextareaInput(this.placeholder);
      }
      // Range Input
      ...
      // HTML5 Input type
        ...
        this.input_type = this.format;
        this.input = this.theme.getFormInputField(this.input_type, this.placeholder);
      }
    }
    // Normal text input
    else {
    ...
    if (this.schema.hasInheritance) {
        this.always_disabled = this.schema.isInherited;
        this.input.disabled = this.schema.isInherited;
        this.inheritanceButton = this.theme.getInheritanceButton(this.schema.isInherited, this.path);
    }
    ...
    this.input
      .addEventListener('change',function(e) {
        e.preventDefault();
        e.stopPropagation();
        self.handleChangeEvent(self, this);
      });
    ...
    this.control = this.theme.getFormControl(this.label, this.input, this.description, this.inheritanceButton);
    if (this.extraButtons) {
        $each(this.extraButtons, function(key, button) {
...
JSONEditor.defaults.editors.object = JSONEditor.AbstractEditor.extend({
  ...
  build: function() {
    var self = this, isMapObject = this.schema.hasOwnProperty("patternProperties"),
        isGrouping = this.schema.hasOwnProperty("properties");
    ...
    // If the object should be rendered as a div
      ...
      this.header = this.theme.getHeader(this.getTitle());
      this.title = this.theme.getGridContainer();
      if (isMapObject) {
          this.header = this.theme.getMapHeader(this.getTitle());
      }
      if ((isGrouping || isMapObject) && this.schema.title) {
      ...
      this.addproperty_input_value = this.theme.getValueFormInputField();
      this.addproperty_input_value_wrap = this.theme.getFirstColumnWrapper();
      this.addproperty_input_value_wrap.appendChild(this.addproperty_input_value);


      this.addproperty_input_key = this.theme.getKeyFormInputField();
      this.addproperty_input_wrap = this.theme.getSecondColumnWrapper();
      this.addproperty_input_wrap.appendChild(this.addproperty_input_key);

      var addItem = function (e) {
          e.preventDefault();
          e.stopPropagation();
          if(self.addproperty_input_key.value) {
            if(self.editors[self.addproperty_input_key.value]) {
              self.theme.addError(self.addproperty_holder);
              return;
            }
            self.theme.removeError(self.addproperty_holder);
            var editor = self.addObjectProperty(self.addproperty_input_key.value);
            if (editor) {
                editor.setValue(self.addproperty_input_value.value);
            }
            self.addproperty_input_key.value = '';
            self.addproperty_input_value.value = '';
            self.onChange(true);
          }
      };
      this.addproperty_add.addEventListener('click', addItem);
      this.addproperty_input_key.addEventListener('keyup',function(e) {
        self.theme.removeError(self.addproperty_holder);
        if (e.keyCode !== 13) {
            return;
        }
      ...
      this.addproperty_input_value.addEventListener('keyup',function(e) {
        self.theme.removeError(self.addproperty_holder);
        if (e.keyCode !== 13) {
            return;
        }
      ...
      if (isMapObject) {
          this.theme.addBorder(this.container);
          this.addproperty_holder.appendChild(this.addproperty_input_wrap);
          this.addproperty_holder.appendChild(this.addproperty_input_value_wrap);
      ...
      $each(this.editors, function(key,editor) {
        var holder = self.theme.getGridColumn(), deleteButton;
        self.row_container.appendChild(holder);

        editor.addDeleteButton = true;
        editor.setContainer(holder);
        editor.build();
        editor.postBuild();
      });
      ...
      this.addproperty_controls = this.theme.getGridRow();
      if (isMapObject) {
          this.container.appendChild(this.addproperty_controls);
      }
  ...
  addObjectProperty: function(name, prebuild_only) {
    ...
    // New property
      ...
      if(!prebuild_only) {
        ...
        self.editor_holder.appendChild(holder);
        if (self.addDeleteButton) {
            var deleteButton = self.getInlineButton('','delete','delete');
            deleteButton.addEventListener('click',function(e) {
              e.preventDefault();
              e.stopPropagation();
              self.removeObjectProperty(name);
            });
            self.editors[name].addButton(deleteButton);
        }
  ...
  refreshAddProperties: function() {
    ...
    // If additional properties are disabled
      ...
      this.addproperty_input_key.style.display = 'none';
    }
...
JSONEditor.defaults.editors.array = JSONEditor.AbstractEditor.extend({
  ...
  syncModelWithView: function() {
      $each(this.rows, function(i, childEditor) {
        childEditor.syncModelWithView();
      });
  },
  build: function() {
    var self = this;
...
JSONEditor.defaults.editors["enum"] = JSONEditor.AbstractEditor.extend({
  ...
  handleChangeEvent: function(editor, element) {
      editor.selected = editor.select_options.indexOf(element.value);
      editor.value = editor["enum"][editor.selected];
      editor.refreshValue();
      editor.onChange(true);
  },
  build: function() {
...
JSONEditor.defaults.editors.multiselect = JSONEditor.AbstractEditor.extend({
  ...
  handleChangeEvent: function(editor) {
      var new_value = [];
      for(i = 0; i<editor.option_keys.length; i++) {
        if(editor.select_options[editor.option_keys[i]].selected || editor.select_options[editor.option_keys[i]].checked) new_value.push(editor.select_values[editor.option_keys[i]]);
      }

      editor.updateValue(new_value);
      editor.onChange(true);
  },
  build: function() {
    var self = this, i;
...
JSONEditor.defaults.editors.checkbox = JSONEditor.AbstractEditor.extend({
  ...
  handleChangeEvent: function(editor, element) {
      editor.value = element.checked;
      editor.onChange(true);
  },
  build: function() {
    var self = this, inputId = this.theme.getInputId();
    if(!this.options.compact) {
...
JSONEditor.defaults.editors.arraySelectize = JSONEditor.AbstractEditor.extend({
  build: function() {
    var inputId = this.theme.getInputId();
    ...
    if (inputId) {
        this.title.setAttribute("for", inputId);
        window.jQuery(group).find("input")[0].setAttribute("id", inputId);
    }
  },
  syncModelWithView: function() {
      this.refreshValue();
