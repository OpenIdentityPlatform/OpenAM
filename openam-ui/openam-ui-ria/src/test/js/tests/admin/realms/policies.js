...
define("tests/admin/realms/policies", [
], function () {
    return {
        executeAll: function () {
            /*
            TODO: tests below are not working
            ...
            QUnit.asyncTest("Edit Application", function () {
                ...
                EditAppView.render(["iPlanetAMWebAgentService"], function () {
                    ...
                    QUnit.ok(!EditAppView.$el.find("#reviewName").parents(".review-row").hasClass(".invalid"),
                        "Validate isn't displayed in the review step");
                    ...
                    if ((!EditAppView.$el.find("#reviewDescr").html()) && (!entity.description)) {
                        ...
                        QUnit.ok(EditAppView.$el.find("#reviewDesc").html().trim() === (entity.description ? entity.description : ""),
            ...
            QUnit.asyncTest("Edit Policy: Resources", function () {
                ...
                EditPolicyView.render([appName, policyName], function () {
                    ...
                    _.each(createdResources, function (item) {
                        correctResources = correctResources && $(item).text().trim() === $(item).data().resource &&
                            _.contains(entity.resources, $(item).data().resource);
                    });
                    ...
                    CreatedResourcesView.render(EditPolicyView.data, function () {
                        ...
                        _.each(listItems, function (item) {
                            valid = valid && $(item).text().trim() === $(item).data().resource
                                && _.contains(entity.resources, $(item).data().resource);
                        });
                        ...
                        QUnit.ok(entity.resources.length === resourceLength - 1 &&
                            !_.contains(entity.resources, lastAddedItem.data().resource), "Resource deleted");
            ...
            QUnit.asyncTest("Edit Policy: User Attributes", function () {
                ...
                EditPolicyView.render([appName, policyName], function () {
                    $.when(PolicyDelegate.getAllUserAttributes()).done(function () {
                        ...
                        QUnit.ok(!_.find(EditPolicyView.staticAttrsView.data.items, { propertyName: "incompleteVal" }),
                            "Static attributes with no key can't be added");
                        editing = EditPolicyView.staticAttrsView.$el.find(".editing");
                        key = editing.find("[data-attr-key]");
                        val = editing.find("[data-attr-val]");
                        addBtn = editing.find(".fa-plus");
                        ...
                        QUnit.ok(!_.find(EditPolicyView.staticAttrsView.data.items, { propertyName: "incompleteKey" }),
                            "Static attributes with no value can't be added");

                        QUnit.equal(EditPolicyView.$el.find("#userAttrs").find(".selectize-input").find(".item").length,
                            userAttributes.length, "User attributes are selected correctly");
                        QUnit.start();
                    });
                });
            });
            ...
            QUnit.asyncTest("Edit Policy: Actions", function () {
                ...
                EditPolicyView.render([appName, policyName], function () {
                    ...
                    row.trigger("click");
                    QUnit.equal(EditPolicyView.data.options.availableActions.length, resourceTypesActionsInitialLength - 1,
                        "Minus one available action");
            ...
            QUnit.asyncTest("Create New Resource Type", function () {
                ...
                EditResourceTypeView.render([], function () {
                    ...
                    QUnit.ok(EditResourceTypeView.$el.find("#reviewName").hasClass("text-danger"),
                        "Name field is marked as invalid");
                    QUnit.equal(EditResourceTypeView.$el.find("#reviewRes").length, 0,
                        "Resources field is not displayed in review step as it is invalid");
                    QUnit.ok(EditResourceTypeView.$el.find("[name='submitForm']").is(":disabled"),
            ...
            QUnit.asyncTest("Edit Resource Type", function () {
                ...
                EditResourceTypeView.render([resTypeUUID], function () {
                    ...
                    // Step 3
                    QUnit.ok(EditResourceTypeView.$el.find("#reviewName").text().trim() === entity.name,
                        "Correct name is displayed in the review step");
