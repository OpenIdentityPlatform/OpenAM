 ...
 */
public class ListAgents extends AuthenticatedCommand {
    /**
    ...
    public void handleRequest(RequestContext rc) 
        ...
        writeLog(LogWriter.LOG_ACCESS, Level.INFO, "ATTEMPT_LIST_AGENTS",
            params);
            ...
            if ((results != null) && !results.isEmpty()) {
                for (Iterator i = results.iterator(); i.hasNext(); ) {
                    ...
                    outputWriter.printlnMessage(MessageFormat.format(
                        getResourceString("format-search-agent-results"),args));
                }
            } else {
                outputWriter.printlnMessage(getResourceString(
                    "search-agent-no-entries"));
            }

            writeLog(LogWriter.LOG_ACCESS, Level.INFO, 
                "SUCCEED_LIST_AGENTS", params);
        } catch (IdRepoException e) {
            String[] args = {realm, patternType, filter, e.getMessage()};
            debugError("ListAgents.handleRequest", e);
            writeLog(LogWriter.LOG_ERROR, Level.INFO,
                "FAILED_LIST_AGENTS", args);
