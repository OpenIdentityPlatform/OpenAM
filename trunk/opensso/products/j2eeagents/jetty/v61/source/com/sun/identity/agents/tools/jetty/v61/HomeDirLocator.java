...
public class HomeDirLocator implements IServerHomeDirLocator, IConstants,
    ...
    public String getServerDirectory(IStateAccess state)
        ...
        if (state != null) {
            jettyConfigDir = (String) state.get(
                    STR_KEY_JETTY_SERVER_CONFIG_DIR);
        ...
        if (!FileUtils.isDirValid(jettyHomeDir)) {
            Debug.log(
                "HomeDirLocator: The Jetty home directory "
                + jettyHomeDir + " specified is invalid:");
            throw new InstallException(
                    LocalizedMessage.get(JETTY_ERR_IN_VALID_HOME_DIR, 
                    STR_JETTY_GROUP));
        } 
        Debug.log(
            "HomeDirLocator : Jetty server home " + "directory = "
            + jettyHomeDir);
        return jettyHomeDir;
    }
}
