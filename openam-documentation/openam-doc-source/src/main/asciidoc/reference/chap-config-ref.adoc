...
If enabled, every request using the authorization code grant must include a `code_challenge` parameter at the authorization endpoint and a matching `code_verifier` parameter at the token endpoint (Proof Key for Code Exchange, PKCE).
