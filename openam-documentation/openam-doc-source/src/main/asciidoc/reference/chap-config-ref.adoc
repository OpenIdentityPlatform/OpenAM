...
[NOTE]
======
Before using a PKCS#11 keystore, make sure your Java runtime environment supports it. For more information, see the link:https://docs.oracle.com/en/java/javase/11/security/pkcs11-reference-guide1.html[JDK 11 PKCS#11 Reference Guide., window=\_blank]
======
+
...
[NOTE]
====
The Liberty ID-WSF SOAP endpoints (`/Liberty/*` and `/WSPRedirectHandler/*`) are disabled by default. To enable them, set the advanced server property `com.sun.identity.liberty.enabled` to `true`. Two additional advanced properties (`com.sun.identity.liberty.soap.sensitiveHandlers` and `com.sun.identity.liberty.allowAnonymousWSC`) control how anonymous and null-family authentication is handled by sensitive Liberty handlers (IDPP, Discovery, AuthnSvc, Interaction). See xref:#servers-advanced-configuration["Advanced"].
====

[NOTE]
====
The Liberty ID-FF SOAP endpoint (`/SOAPReceiver/*`, served by `FSSOAPReceiver`) is disabled by default. To enable it, set the advanced server property `com.sun.identity.federation.services.soap.enabled` to `true`. This kill-switch covers the whole endpoint, including ID-FF browser-artifact single sign-on and LECP, not only the state-changing operations.

Independently of that kill-switch, inbound ID-FF state-changing and disclosure operations (federation termination, name registration, single logout, and name-identifier mapping) require a verified XML signature unless `com.sun.identity.federation.services.requireSignature` is set to `false` (not recommended). That requirement applies to both the SOAP binding and the HTTP-Redirect binding (`/ProcessTermination/*`, `/ProcessLogout/*`, and `/ProcessRegistration/*`), and OpenAM only signs its own outbound ID-FF messages when `XMLSigningOn` is enabled. See xref:#servers-advanced-configuration["Advanced"].
====
...
If enabled, every request whose `response_type` includes `code` must supply a `code_challenge` parameter at the authorization endpoint and a matching `code_verifier` parameter at the token endpoint (Proof Key for Code Exchange, PKCE). Enforcement covers the plain authorization code grant and the OpenID Connect hybrid flows (`code token`, `code id_token`, and `code id_token token`) alike, because every one of them issues an authorization code.
...
[NOTE]
====
Independently of this setting, if an authorization code was issued together with a `code_challenge`, OpenAM always requires and validates the corresponding `code_verifier` when the code is exchanged at the token endpoint, as mandated by link:https://tools.ietf.org/html/rfc7636#section-4.6[RFC 7636 §4.6, window=\_top]. Disabling this property only stops OpenAM from *forcing* all clients to use PKCE; it never relaxes verification for codes that were issued with a challenge. This prevents PKCE downgrade attacks.
====

+
If the `code_challenge_method` parameter is omitted, it defaults to `plain`, as specified in link:https://tools.ietf.org/html/rfc7636#section-4.3[RFC 7636 §4.3, window=\_top]. OpenAM supports `plain` and `S256`. While this property is enabled, the authorization endpoint rejects any other value; an explicitly empty value is treated as malformed rather than as an omitted one. Prefer `S256`: link:https://tools.ietf.org/html/rfc7636#section-7.2[RFC 7636 §7.2, window=\_top] states that `plain` SHOULD NOT be used, and retains it only for clients that cannot compute a SHA-256 digest.

+
This property governs the realm's OAuth2 provider. Clients served by an agent provider profile are never forced to use PKCE, whatever this property is set to; a `code_challenge` that such a client does send is still bound to the code and verified at the token endpoint.

+
For more information, see link:https://tools.ietf.org/html/rfc7636[Proof Key for Code Exchange by OAuth Public Clients, window=\_top] - __RFC 7636__.
...
[NOTE]
==========
This feature only takes effect if the security manager is enabled for the JVM. The JVM security manager is deprecated, disabled by default, and cannot be installed on recent Java versions, so this setting may have no effect; do not rely on it as the sole safeguard. For Groovy scripts, compile-time AST transformation annotations (such as `@Grab` and `@ASTTest`) are rejected during compilation regardless of this setting.
==========
+
...
+
Default: Derived from the deployment URL scheme at install time. If the server is configured over HTTPS, the secure flag is enabled (`Yes`); over plain HTTP it is left disabled (`No`) with a deploy-time warning.

+
property: `com.iplanet.am.cookie.secure`

SameSite Cookie::
Sets the `SameSite` attribute on the cookies issued by OpenAM. Valid values are `Lax`, `Strict`, and `None`.

+
Default: `Lax`

+
property: `org.openidentityplatform.openam.cookie.samesite`
...
Master kill-switch for the legacy Liberty ID-FF (Liberty Alliance 1.x) SOAP endpoint (`/SOAPReceiver/*` served by `FSSOAPReceiver`).

+
When `false` (the default), every request to this endpoint is rejected with HTTP 404. Liberty ID-FF is a legacy federation protocol and this endpoint is reachable without authentication, so it is disabled unless a deployment actively uses ID-FF federation. The kill-switch covers every operation served by the endpoint, including ID-FF browser-artifact single sign-on and LECP. The property is read on every request, so the endpoint can be enabled or disabled without restarting OpenAM.

+
Default: `false`

`com.sun.identity.federation.services.requireSignature`::
Controls whether inbound Liberty ID-FF requests for state-changing or disclosure operations (federation termination, name registration, single logout, and name-identifier mapping) must carry a verified XML signature. Applies to both the SOAP binding (`/SOAPReceiver/*`) and the HTTP-Redirect binding (`/ProcessTermination/*`, `/ProcessLogout/*`, and `/ProcessRegistration/*`). These endpoints are reachable without authentication and act on the user named by an opaque handle in the request, so the signature is what identifies the caller.

+
When `true` (the default), unsigned requests for these operations are rejected even when message signing is otherwise optional or off (see `XMLSigningOn` under `sunFAMIDFFConfiguration`, exposed as `com.sun.identity.federation.services.signingOn`). Because OpenAM only signs its own outbound ID-FF messages when that setting is enabled, a deployment that leaves this property at its default must also enable `XMLSigningOn` and configure a signing certificate on every participating provider; otherwise OpenAM's own single logout, federation termination, and name registration messages are rejected by the receiving provider. Set this property to `false` only to restore the legacy (insecure) behaviour required by deployments whose federation partners do not sign ID-FF messages.

+
Only the exact value `false` disables the requirement; any other value, including a malformed one, leaves it enabled.
...
Controls whether the Liberty ID-WSF SOAP receiver mints a synthetic anonymous Web Service Client (WSC) session for null-family authentication mechanisms (`urn:liberty:security:2003-08:null:*`) when no client credential (TLS or message certificate) is presented.

+
When `false` (the default), such requests are rejected (fail-close). Set to `true` only to restore the legacy (insecure) behaviour required by deployments that explicitly rely on anonymous WSC sessions. This setting was introduced as part of the GHSA-p462-xxwx-pqf4 hardening.

+
Default: `false`

`com.sun.identity.liberty.enabled`::
Master kill-switch for the legacy Liberty ID-WSF SOAP endpoints (`/Liberty/*` served by `SOAPReceiver` and `/WSPRedirectHandler/*` served by `WSPRedirectHandlerServlet`).

+
When `false` (the default), every request to these endpoints is rejected with HTTP 404. Liberty ID-WSF is a legacy protocol and most deployments do not need it. Set to `true` only if your deployment actively uses Liberty ID-WSF, IDPP, Discovery, AuthnSvc, or Interaction services.

+
The property is read on every request, so the endpoint can be enabled or disabled without restarting OpenAM.

+
Default: `false`

`com.sun.identity.liberty.soap.sensitiveHandlers`::
Comma-separated list of Liberty SOAP `RequestHandler` keys that MUST NOT accept the unauthenticated `urn:liberty:security:2003-08:null:null` mechanism nor the ANONYMOUS security profile, regardless of binding-level `SupportedAuthenticationMechanisms` or `SecurityProfile` configuration.

+
Empty value disables the additional restriction (not recommended). This setting was introduced as part of the GHSA-p462-xxwx-pqf4 hardening.
...
Controls whether the SSO token id (`tokenId`) is returned in the `/json/authenticate` response body when the session cookie is HttpOnly.

+
This property is only relevant when the session cookie is HttpOnly (`com.sun.identity.cookie.httponly=true`). By default (`false`), the token is delivered to the browser solely through the `Set-Cookie` header and is not echoed in the response body, so an XSS on the origin cannot read a replayable SSO token (including a freshly upgraded one). The XUI relies on the auto-sent cookie and does not need the token in the body.

+
Set this property to `true` to keep the legacy behaviour of also returning `tokenId` in the response body, for example for non-browser or raw-REST integrations that require both the HttpOnly cookie and the token in the body.
...
Whitelist of URL patterns that the SAML 2.0 and ID-FF IDP discovery reader and writer services (`/saml2reader`, `/saml2writer`, `/idffreader`, `/idffwriter`, served by `CookieReaderServlet` and `CookieWriterServlet`) are allowed to redirect to through the `RelayState` parameter.

+
To prevent these unauthenticated endpoints from being abused as an open redirect (GHSA-2pf8-52jh-5x3m), the value of `RelayState` is validated before any redirect. Relative URLs, and URLs that target the same scheme, host, and port as the incoming request, are always allowed. Any other absolute URL is rejected with HTTP 400 unless it matches one of the patterns configured here.

+
Specify a semicolon-separated list of URL patterns, using `*` as a wildcard, for example `https://sp.example.com:443/*;https://sp2.example.com/*`. Add the return URLs of any service providers that drive discovery from a different host than OpenAM. This setting was introduced as part of the GHSA-2pf8-52jh-5x3m hardening.

+
Default: empty (only relative and same-origin redirect URLs are allowed)
