...
+
The `/ws/1/entitlement/listener` resource registers a callback URL that OpenAM sends an HTTP POST to whenever an entitlement policy changes. To prevent server-side request forgery (link:https://github.com/OpenIdentityPlatform/OpenAM/security/advisories/GHSA-g499-5qm8-4grm[GHSA-g499-5qm8-4grm, window=\_blank]), a registered listener URL must use the `http` or `https` scheme and must not resolve to a blocked address; unsafe URLs are rejected at registration and are never contacted when notifications are sent. The blocked addresses are loopback, wildcard, link-local (including the cloud metadata range `169.254.0.0/16`), private/site-local, unique-local and multicast addresses, the IPv4 special-purpose ranges `0.0.0.0/8`, `100.64.0.0/10`, `192.0.0.0/24`, `192.88.99.0/24`, `198.18.0.0/15` and `240.0.0.0/4`, the NAT64 local-use prefix `64:ff9b:1::/48`, and any IPv6 transition address (NAT64, 6to4, Teredo, ISATAP or IPv4-mapped) that embeds one of those (link:https://github.com/OpenIdentityPlatform/OpenAM/security/advisories/GHSA-xq25-2x9w-94w9[GHSA-xq25-2x9w-94w9, window=\_blank]); the full list is in xref:../admin-guide/chap-openid-connect.adoc#openam-ssrf-blocked-addresses["OpenID Connect Relying Party Registration"] in the __Administration Guide__. Deployments that run a NAT64 translator with a Network-Specific Prefix declare it in the advanced server property `org.openidentityplatform.ssrf.nat64-prefixes`, as described there. To restore the previous, unrestricted behavior at runtime — for example, when the listener endpoint is hosted on an internal address — set the advanced server property `org.openidentityplatform.entitlement.listener.allow-any-url` to `true`.
+
[IMPORTANT]
======
A listener registered before this check existed — or before the blocked list grew to cover ranges such as the `100.64.0.0/10` shared address space used by some container platforms and carrier networks — stays in the configuration but stops receiving notifications after an upgrade. The listener is skipped one notification at a time, and each skip is logged to the `Entitlement` debug file as `PrivilegeChangeNotifier.notify: skipping unsafe listener URL`. If your listener endpoints are intentionally on internal addresses, set `org.openidentityplatform.entitlement.listener.allow-any-url` to `true` before upgrading.
======
...
+
As of link:https://github.com/OpenIdentityPlatform/OpenAM/security/advisories/GHSA-w858-46wv-v45w[OpenAM Security Advisory GHSA-w858-46wv-v45w, window=\_blank], the notification-URL registration methods exposed under `/jaxrpc/*` (`registerNotificationURL` and `registerNotificationURL_idrepo`) require the caller to be authenticated as a server or an agent. The caller's SSO token is read from the request cookie and must be an administrative (server) token or an application (agent) session; otherwise the registration is rejected. This prevents an unauthenticated caller from registering an arbitrary callback URL and turning the endpoint into a persistent server-side request forgery (SSRF) vector, whereby OpenAM would issue HTTP POST requests to that URL on every configuration change. To restore the previous, unauthenticated behavior at runtime (for example, for legacy remote clients that do not send an SSO cookie), set the advanced server property `org.openidentityplatform.notification.register.skip-auth-check` to `true`.
...
+
A logout request received from a remote provider is processed without a local session, so it must carry a verified XML signature unless the advanced server property `com.sun.identity.federation.services.requireSignature` is set to `false`. See xref:../reference/chap-config-ref.adoc#servers-advanced-configuration["Advanced"] in the server configuration reference.
...
+
This endpoint is disabled by default and returns HTTP 404 for every request unless the advanced server property `com.sun.identity.federation.services.soap.enabled` is set to `true`. See xref:../reference/chap-config-ref.adoc#servers-advanced-configuration["Advanced"] in the server configuration reference.
...
+
This endpoint is unauthenticated and defederates the user named in the request, so the request must carry a verified XML signature unless the advanced server property `com.sun.identity.federation.services.requireSignature` is set to `false`. See xref:../reference/chap-config-ref.adoc#servers-advanced-configuration["Advanced"] in the server configuration reference.
...
+
This endpoint is unauthenticated and rewrites the name identifier of the user named in the request, so the request must carry a verified XML signature unless the advanced server property `com.sun.identity.federation.services.requireSignature` is set to `false`. See xref:../reference/chap-config-ref.adoc#servers-advanced-configuration["Advanced"] in the server configuration reference.
...
+
This endpoint is disabled by default and returns HTTP 404 for every request unless the advanced server property `com.sun.identity.liberty.enabled` is set to `true`. See xref:../reference/chap-config-ref.adoc#servers-advanced-configuration["Advanced"] in the server configuration reference.
...
+
This endpoint is also gated by the advanced server property `com.sun.identity.liberty.enabled` (default `false`).
...
+
This endpoint redirects to the URL supplied in the `RelayState` parameter. To prevent it from being abused as an open redirect (GHSA-2pf8-52jh-5x3m), only relative URLs, same-origin URLs, and URLs matching the advanced server property `org.openidentityplatform.openam.idpdiscovery.relaystate.whitelist` are accepted; other targets are rejected with HTTP 400. See xref:../reference/chap-config-ref.adoc#servers-advanced-configuration["Advanced"] in the server configuration reference.
...
+
This endpoint also redirects to the URL supplied in the `RelayState` parameter, and applies the same redirect-URL validation described above for the writer endpoints.
...
`/ccversion/*`::
Associated with the servlet named `VersionServlet`. The associated `VersionServlet.java` file forwards every request to `/WEB-INF/jsp/Version.jsp`, which renders the console version window.
