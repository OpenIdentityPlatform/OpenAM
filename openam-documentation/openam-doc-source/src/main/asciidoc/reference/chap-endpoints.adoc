...
+
The `/ws/1/entitlement/listener` resource registers a callback URL that OpenAM sends an HTTP POST to whenever an entitlement policy changes. To prevent server-side request forgery (link:https://github.com/OpenIdentityPlatform/OpenAM/security/advisories/GHSA-g499-5qm8-4grm[GHSA-g499-5qm8-4grm, window=\_blank]), a registered listener URL must use the `http` or `https` scheme and must not resolve to a loopback, link-local (including the cloud metadata range `169.254.0.0/16`), private/site-local, multicast, or other special-purpose address; unsafe URLs are rejected at registration and are never contacted when notifications are sent. The same rules are applied to the IPv4 address embedded in an IPv6 transition literal, so an internal target cannot be reached through a NAT64, 6to4, Teredo, ISATAP, or IPv4-mapped address either (link:https://github.com/OpenIdentityPlatform/OpenAM/security/advisories/GHSA-xq25-2x9w-94w9[GHSA-xq25-2x9w-94w9, window=\_blank]); deployments that run a NAT64 translator with a Network-Specific Prefix declare it in the advanced server property `org.openidentityplatform.ssrf.nat64-prefixes`, as described in xref:../admin-guide/chap-openid-connect.adoc#register-openid-connect-client-dynamic["To Register a Relying Party Dynamically"] in the __Administration Guide__. To restore the previous, unrestricted behavior at runtime — for example, when the listener endpoint is hosted on an internal address — set the advanced server property `org.openidentityplatform.entitlement.listener.allow-any-url` to `true`.
