...
* *Supportability*. The Supportability Phase involves creating the runbook for system administrators including procedures for backup and restores, debugging, change control, and other processes. If you have a Open Identity Platform Approved Vendor Support contract, it ensures everything is in place prior to your deployment.
...
Open Identity Platform Approved Vendors offer training courses for OpenAM topics, including OpenAM development and deployment. For a current list of available courses, contact https://github.com/OpenIdentityPlatform/.github/wiki/Approved-Vendor-List[Approved Vendor List, window=\_blank].
...
As for other testing, take time to define acceptance criteria. Know that Open Identity Platform Community has performed penetration testing on the software for each enterprise release. Any customization, however, could be the source of security weaknesses, as could configuration to secure OpenAM.
...
Also, plan back line support with Open Identity Platform Community Approved Vendor. The aim is to define clearly who handles production issues, and how production issues are escalated to a product specialist if necessary.
