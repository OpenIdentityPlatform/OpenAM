...
[NOTE]
======
The JVM security manager is deprecated, disabled by default, and cannot be installed at all on recent Java versions. Do not rely on it as your only safeguard: the whitelist and blacklist remain the primary runtime controls.
======

For Groovy scripts, the scripting engine additionally rejects compile-time AST (Abstract Syntax Tree) transformation annotations, such as `@Grab` and `@ASTTest`. Unlike the whitelist, blacklist, and security manager checks—which apply while a script __runs__—these annotations execute arbitrary code during script __compilation__, which occurs as soon as a script is validated or created, before the script is ever run. A script that uses a blocked transform is rejected with a compilation error, and Grape dependency resolution (the `@Grab` family of annotations) is disabled.
...
Groovy compile-time transforms are blocked.::
The whitelist, blacklist, and security manager apply only while a script __runs__. Groovy also supports AST transformation annotations—such as `@Grab` and `@ASTTest`—that run arbitrary code at __compile__ time, when a script is validated or created. OpenAM blocks these transforms during compilation so that they cannot be used to bypass the runtime security settings. Benign transforms, such as `@Field`, remain available.
