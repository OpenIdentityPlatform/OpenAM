...
import org.forgerock.openam.sts.publish.config.STSPublishModule;
...
public class RestSTSInstancePublisherImpl implements RestSTSInstancePublisher {
    ...
    RestSTSInstancePublisherImpl(Router router,
                                 ...
                                 @Named(STSPublishModule.REST_STS_PUBLISH_LISTENER)ServiceListener serviceListener,
    ...
    public synchronized String publishInstance(RestSTSInstanceConfig instanceConfig, RestSTS restSTSInstance,
        ...
        if (!republish) {
            try {
                persistentStore.persistSTSInstance(deploymentSubPath, instanceConfig.getDeploymentConfig().getRealm(),
                        instanceConfig);
