 ...
 * of this conversion, a {@code Map<String, Object>}, returned from JsonValue.asMap, must be transformed into a {@code Map<String, Set<String>>}.
 * Likewise, when going in the other direction (SMS map to JsonValue), the {@code Map<String, Set<String>>} needs to be converted
 * back to a {@code Map<String, Object>} so that a JsonValue can be constructed therefrom.
 *
...
public class MapMarshallUtils {
    /**
     * Marshals from a {@code Map<String, Object>} produced by jsonValue.asMap to the {@code Map<String, Set<String>>} expected by the
     * SMS.
     ...
     *
     * @param jsonMap the {@code Map<String, Object>} produced by jsonValue.asMap to be marshalled
     ...
     * conversion between String and {@code Set<String>} for non-complex values. I would like to exclude Set instances with
     * a cardinality != 1, but encapsulated complex types will be passed the {@code Map<String, Set<String>>} corresponding to the
     * top-level object when the object hierarchy is re-constituted from SMS state, which means that some Map entries
     * will have {@code Set<String>} instances with a cardinality != 1. Yet this should not affect the leaf objects, as they do
    ...
    /**
     * This method is called by AuthTargetMapping to marshal the contextMap from the {@code Map<String, Object>} returned by the
     * jsonValue.asMap to the {@code Map<String, String>} stored by the AuthTargetMapping. The AuthTargetMapping specifies the
     ...
     * ultimately be stored in the SMS as a String, I cannot store the {@code Map<String, Object>} native to json maps, but rather
     * must marshal this representation to a {@code Map<String, String>}.
