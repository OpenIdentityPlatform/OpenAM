...
public class AuthenticationHandlerImpl<T> implements AuthenticationHandler<T> {
    ...
    public void authenticate(RequestData requestData, T token, ValidationInvocationContext validationInvocationContext,
                             boolean invalidateOpenAMSession) throws TokenValidationException {
        final URL authUrl = authenticationUrlProvider.authenticationUrl(token);
