...
#define	COMMON_SESSION_REQUEST_PREFIX_DATA \
    ...
    const char logoutSessionRequestPrefix[] = {
	    "\">\n"
	      "<Logout"
    };

    const char logoutSessionRequestMiddleSessionID[] = {
	      ">\n"
	      	"<SessionID>"
    };

    const char logoutSessionRequestSuffix[] = {
		"</SessionID>\n"
	      "</Logout>\n"
	    "</SessionRequest>]]>\n"
	  "</Request>\n"
...
const SessionService::BodyChunk
SessionService::logoutSessionRequestPrefixChunk(logoutSessionRequestPrefix,
					         sizeof(logoutSessionRequestPrefix) - 1);

const SessionService::BodyChunk
SessionService::logoutSessionRequestMiddleSessionIDChunk(logoutSessionRequestMiddleSessionID,
					         sizeof(logoutSessionRequestMiddleSessionID) - 1);

const SessionService::BodyChunk
SessionService::logoutSessionRequestSuffixChunk(logoutSessionRequestSuffix,
					         sizeof(logoutSessionRequestSuffix) - 1);
const SessionService::BodyChunk
 ...
 */
am_status_t
SessionService::parseLogoutSessionResponse(XMLElement element,
					    const std::string& sessionId) const
{
    am_status_t status;

    if (element.isNamed("Logout")) {
	element = element.getFirstSubElement();
	if (element.isNamed("OK")) {
	    Log::log(logModule, Log::LOG_DEBUG,
		     "SessionService::parseLogoutSessionResponse() %s OK ",
		     sessionId.c_str());
	    status = AM_SUCCESS;
	} else if (element.isNamed("Exception")) {
	    status = parseException(element, sessionId);
 ...
 */
am_status_t
SessionService::logoutSession(const ServiceInfo& service, 
                               const std::string& ssoToken, 
                               const Http::CookieList& cookieList)
{
    if (ssoToken.size()==0) 
    ...
    am_status_t status;
    const std::size_t LOGOUT_SESSION_NUM_CHUNKS = 8;
    Request request(*this, prefixChunk, firstSessionRequestPrefixChunk,
		     LOGOUT_SESSION_NUM_CHUNKS);
    Http::Response response;
    BodyChunk ssoTokenChunk(ssoToken);
    BodyChunkList& bodyChunkList = request.getBodyChunkList();

    bodyChunkList.push_back(logoutSessionRequestPrefixChunk);
    bodyChunkList.push_back(logoutSessionRequestMiddleSessionIDChunk);
    bodyChunkList.push_back(ssoTokenChunk);
    bodyChunkList.push_back(logoutSessionRequestSuffixChunk);
    bodyChunkList.push_back(suffixChunk);
	...
	try {
	    ...
	    if (sessionResponses.empty()) {
	        Log::log(logModule, Log::LOG_ERROR,
		         "SessionService::logoutSession() received empty "
                         "responses.");
		throw XMLTree::ParseException(
                    "Session Service logout session returned "
                    "an empty ResponseSet");
	    }
		...
		if (element.isNamed("SessionResponse") &&
		    ...
		    if (std::strcmp(requestId.c_str(),
				    request.getServiceId()) == 0) {
			status = parseLogoutSessionResponse(subElement,
							     ssoToken);
		    } else {
			throw XMLTree::ParseException("unknown request id "
						      "in Session Service "
	    ...
	    Log::log(logModule, Log::LOG_ERROR,
		     "SessionService::logoutSessionInfo() caught exception: %s",
		     exc.getMessage().c_str());
	    status = AM_ERROR_PARSING_XML;
	}
    }
