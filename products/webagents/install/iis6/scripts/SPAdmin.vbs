...
'
' $Id: SPAdmin.vbs,v 1.1.2.3 2010/04/19 22:52:49 subbae Exp $
'
' Copyright 2007 Sun Microsystems Inc. All Rights Reserved
'
'---------------------------------------------------------------------------
' Configures/UnConfigures the Auth filter for SharePoint agent
'
' Requires:
'    -config/-unconfig 
'
' Usage: SPAdmin.vbs -config/-unconfig <config-filename>
'
' Example:
'    SPAdmin.vbs -config agentConfig
'
'
...
Dim WsShell, WSysEnv, ProcArch

filterName= "amspfilter"
...
if Args.Count < 2 Then
   ...
   WScript.Echo "Syntax: SPAdmin.vbs -config/-unconfig <config-filename>"
   WScript.Quit(1)
end if
...
currDir = split(scriptFullName, "\SPAdmin.vbs")
...
Function AgentConfigure(WshShell, FSO, responseFile, objWebRoot, dict1)
   ...
   Set WSysEnv = WsShell.Environment("SYSTEM")
   ProcArch = WSysEnv("PROCESSOR_ARCHITECTURE")

   'Confifure the auth filter 
   if (ProcArch = "x86") then
       filterPath = installDir + "\bin\amsharepointfilter32.dll"
       elseif (ProcArch = "AMD64") then
       filterPath = installDir + "\bin\amsharepointfilter64.dll"
       else
       WScript.Echo "Processor architecture not supported"
	   WScript.Quit(1)
   end if
   ...
   If filtFound > 0 Then
       WScript.Echo "Filter already loaded"
	   WScript.Quit(1)
   End If	
   If loadOrder <> "" Then
   	   filtSepr = ","
