...
'
' $Id: OwaAdmin.vbs,v 1.1.2.2 2010/04/19 22:52:49 subbae Exp $
'
' Copyright 2007 Sun Microsystems Inc. All Rights Reserved
'
'---------------------------------------------------------------------------
' Configures/UnConfigures the Auth filter for OWA agent
'
' Requires:
'    -config/-unconfig 
'
' Usage: OwaAdmin.vbs -config/-unconfig <config-filename>
'
' Example:
'    OwaAdmin.vbs -config agentConfig
'
'
...
if Args.Count < 2 Then
   ...
   WScript.Echo "Syntax: OwaAdmin.vbs -config/-unconfig <config-filename>"
   WScript.Quit(1)
end if
...
currDir = split(scriptFullName, "\OwaAdmin.vbs")
...
Function AgentConfigure(WshShell, FSO, responseFile, objWebRoot, dict1)
   ...
   filterPath = installDir + "\bin\amowafilter64.dll"  			
