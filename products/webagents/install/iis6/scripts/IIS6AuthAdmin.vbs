...
'
'---------------------------------------------------------------------------
' Configures/UnConfigures the Auth filter for a Web Site
'
' Requires:
'    -config/-unconfig 
'
' Usage: IIS6AuthAdmin.vbs -config/-unconfig <config-filename>
'
' Example:
'    IIS6AuthAdmin -config agentConfig
'
'
...
if Args.Count < 2 Then
   ...
   WScript.Echo "Syntax: IIS6AuthAdmin.vbs -config/-unconfig <config-filename>"
   WScript.Quit(1)
end if
...
currDir = split(scriptFullName, "\IIS6AuthAdmin.vbs")
...
Function AgentConfigure(WshShell, FSO, responseFile, objWebRoot, dict1)
   ...
   filterPath = installDir + "\bin\amiis6auth.dll"  			
