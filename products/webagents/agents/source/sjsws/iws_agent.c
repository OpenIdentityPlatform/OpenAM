    ...
    if(ret == AM_SUCCESS && redirect_url != NULL) {
        ...
        if ((am_rev_number != NULL) &&
            ...
            am_web_log_debug("%s: policy status = %s, redirection URL is %s",
                          thisfunc, am_status_to_string(status), redirect_url);

            // we need to modify the redirect_url with the policy advice
            if ((am_rev_number != NULL) &&
                      (!strcmp(am_rev_number,"7.0")) &&
                      (B_TRUE == am_web_use_redirect_for_advice()) &&
                      (policy_result->advice_string != NULL)) {
...
*/
static int create_buffer_withpost(const char *key, 
                               am_web_postcache_data_t postentry,
                               Session *sn, Request *rq)
{
    ...
    // session mode is URL.
    status = am_web_get_postdata_preserve_lbcookie(&lbCookieHeader, B_TRUE);
...
validate_session_policy(pblock *param, Session *sn, Request *rq) {
    ...
    // Remove sunwMethod parameter from query if it is there
    if (status == AM_SUCCESS) {
        if((am_web_use_sunwmethod() == B_TRUE) && (query != NULL)) {
            status = am_web_get_parameter_value(query, REQUEST_METHOD_TYPE,
                                            &orig_req);
            if (status == AM_SUCCESS) {
                am_web_log_debug("%s: orig_req: %s", thisfunc, *orig_req);
                status = am_web_remove_parameter_from_query(query,
                                      REQUEST_METHOD_TYPE, &query);
                am_web_log_debug("%s: Query set to %s", thisfunc, query);
            }
        }
    }
    // Get the request URL
    if (status == AM_SUCCESS) {
        ...
        status = am_web_get_request_url(host_hdr, protocol, server_hostname,
                        server_portnum, uri_hdr, query, &request_url);
        if(status == AM_SUCCESS) {
    ...
    if( status == AM_SUCCESS) {
        ...
        {
            boolean_t doCheck = B_FALSE;
            // For not enforced url this check should not be done, unless 
            // it is the dummy post url.
            if ((am_web_is_dummypost_url(request_url, pathInfo_hdr)
                                         == B_TRUE) ||
            ...
            if(((useSunwMethod == B_FALSE) && (dpro_cookie == NULL)
                                           && (doCheck == B_TRUE)) ||
               ((useSunwMethod == B_TRUE) && (orig_req != NULL)
                                          && (strlen(orig_req) > 0)))
            {
                if (useSunwMethod == B_FALSE) {
                    //Set original method to GET
                    if (orig_req != NULL) {
                        am_web_free_memory(orig_req);
                    }
